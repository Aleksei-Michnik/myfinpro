 ...
 * `TRANSACTION_SCHEDULE_MIN_INTERVAL_MS`, creates a `RECURRING` parent transaction
 * with a personal + group attribution, posts a schedule, and waits for the
 * worker to materialise child Transactions.
 *
 * Production minimum stays at 60 000 ms (asserted by the production-default
 * branch in the unit suite).
 */
describe('TransactionOccurrence worker (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.TRANSACTION_SCHEDULE_MIN_INTERVAL_MS = '200';
  ...
  async function waitForOccurrences(
    ...
    while (Date.now() - start < timeoutMs) {
      const got = await prisma.transaction.count({ where: { parentTransactionId: parentId } });
      if (got >= count) return;
      await new Promise((r) => setTimeout(r, 100));
    }
    throw new Error(
      `Timed out waiting for ${count} occurrences on parent ${parentId} (got ${await prisma.transaction.count(
        { where: { parentTransactionId: parentId } },
      )})`,
    );
  }

  it('produces 2 child Transactions with cloned attributions, audit logs, and updated lastRunAt; DELETE stops further firings', async () => {
    ...
    for (const occ of occurrences) {
      ...
      expect(occ.parentTransactionId).toBe(parent.id);
    ...
    const sched = await prisma.transactionSchedule.findUnique({ where: { id: scheduleId } });
    expect(sched?.lastRunAt).not.toBeNull();
    expect(sched?.nextRunAt).not.toBeNull();

    // Audit logs.
    const audits = await prisma.auditLog.findMany({
      where: { action: 'TRANSACTION_OCCURRENCE_CREATED', entityId: parent.id },
    });
    ...
    const countAfterWait = await prisma.transaction.count({
      where: { parentTransactionId: parent.id },
    });
