...
/**
 * Phase 6.10 — Payment comments CRUD + soft-delete integration tests.
 */
describe('Payment comments API (integration)', () => {
  ...
  // ── 1 ──
  it('1. author creates a comment on own personal payment → 201, body matches', async () => {
    const { id: pid } = await createPayment(alice.accessToken, basePayload());
    const res = await request(app.getHttpServer())
      .post(`/api/v1/payments/${pid}/comments`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .send({ content: 'first' })
      .expect(201);
    expect(res.body.content).toBe('first');
    expect(res.body.paymentId).toBe(pid);
  ...
  it('2. GET list returns the new comment oldest-first', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(201);
    const res = await request(app.getHttpServer())
      .get(`/api/v1/payments/${pid}/comments`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
  ...
  // ── 5 ──
  it('5. group payment: member B comments, member C lists and sees both', async () => {
    const { id: pid } = await createPayment(
      alice.accessToken,
  ...
  it('6. author edits own comment → 200, updatedAt > createdAt', async () => {
    ...
    const upd = await request(app.getHttpServer())
      .patch(`/api/v1/payments/${pid}/comments/${c.body.id}`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .send({ content: 'v2' })
      .expect(200);
  ...
  // ── 7 ──
  it("7. member B tries to edit A's comment → 403 PAYMENT_COMMENT_NOT_AUTHOR", async () => {
    const { id: pid } = await createPayment(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );
    ...
    expect(r.body.errorCode).toBe('PAYMENT_COMMENT_NOT_AUTHOR');
  });
  ...
  it('8. author soft-deletes own comment → 204; list excludes it; DB row persists with empty content', async () => {
    ...
    const c = await request(app.getHttpServer())
      ...
      .expect(201);
    await request(app.getHttpServer())
      .delete(`/api/v1/payments/${pid}/comments/${c.body.id}`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(204);
    ...
    const row = await prisma.paymentComment.findUnique({ where: { id: c.body.id } });
  ...
  // ── 9 ──
  it('9. editing an already-soft-deleted comment → 410 PAYMENT_COMMENT_DELETED', async () => {
    const { id: pid } = await createPayment(alice.accessToken, basePayload());
    const c = await request(app.getHttpServer())
      .post(`/api/v1/payments/${pid}/comments`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
    ...
    expect(r.body.errorCode).toBe('PAYMENT_COMMENT_DELETED');
  });

  // ── 10 ──
  it('10. PATCH a non-existent comment under a visible payment → 404', async () => {
    const { id: pid } = await createPayment(alice.accessToken, basePayload());
    const r = await request(app.getHttpServer())
      .patch(`/api/v1/payments/${pid}/comments/00000000-0000-0000-0000-000000000000`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .send({ content: 'x' });
    expect(r.status).toBe(404);
    expect(r.body.errorCode).toBe('PAYMENT_COMMENT_NOT_FOUND');
  });
  ...
  it('11. cursor pagination: 5 comments / limit=2 → 3 pages', async () => {
    ...
    const p1 = await request(app.getHttpServer())
      .get(`/api/v1/payments/${pid}/comments?limit=2`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
    ...
    const p2 = await request(app.getHttpServer())
      .get(
        `/api/v1/payments/${pid}/comments?limit=2&cursor=${encodeURIComponent(p1.body.nextCursor)}`,
      )
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
    ...
    const p3 = await request(app.getHttpServer())
      .get(
        `/api/v1/payments/${pid}/comments?limit=2&cursor=${encodeURIComponent(p2.body.nextCursor)}`,
      )
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
  ...
  // ── 12 ──
  it('12. invalid cursor → 400 PAYMENT_COMMENT_INVALID_CURSOR', async () => {
    const { id: pid } = await createPayment(alice.accessToken, basePayload());
    const r = await request(app.getHttpServer())
      .get(`/api/v1/payments/${pid}/comments?cursor=not-base64`)
      .set('Authorization', `Bearer ${alice.accessToken}`);
    expect(r.status).toBe(400);
    expect(r.body.errorCode).toBe('PAYMENT_COMMENT_INVALID_CURSOR');
  });
  ...
  it('13. cascade: DELETE ?scope=all removes all comments (including soft-deleted)', async () => {
    ...
    const c2 = await request(app.getHttpServer())
      ...
      .expect(201);
    await request(app.getHttpServer())
      .delete(`/api/v1/payments/${pid}/comments/${c2.body.id}`)
      .set('Authorization', `Bearer ${bob.accessToken}`)
      .expect(204);

    const beforeRows = await prisma.paymentComment.findMany({ where: { paymentId: pid } });
    expect(beforeRows).toHaveLength(2);

    // Hard-delete the payment — alice drops the only group attribution via scope=all.
    await request(app.getHttpServer())
      .delete(`/api/v1/payments/${pid}?scope=all`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);

    const afterRows = await prisma.paymentComment.findMany({ where: { paymentId: pid } });
  ...
  it('14. audit trail: create + update + delete writes 3 rows with same entityId', async () => {
    ...
    const audits = await prisma.auditLog.findMany({
      where: {
        ...
        entityId: pid,
        action: {
          in: ['PAYMENT_COMMENT_CREATED', 'PAYMENT_COMMENT_UPDATED', 'PAYMENT_COMMENT_DELETED'],
        },
    ...
    expect(audits.map((a) => a.action)).toEqual([
      'PAYMENT_COMMENT_CREATED',
      'PAYMENT_COMMENT_UPDATED',
      'PAYMENT_COMMENT_DELETED',
    ]);
  });
});
