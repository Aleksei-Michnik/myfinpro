 ...
 */
describe('TransactionSchedule lifecycle (integration)', () => {
  ...
  it('pause → resume → cancel state machine + idempotency guards', async () => {
    ...
    const paused = await request(app.getHttpServer())
      .post(`/api/v1/transactions/${parent.id}/schedule/pause`)
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(200);
    ...
    const pauseAgain = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(409);
    expect(pauseAgain.body.errorCode).toBe('TRANSACTION_SCHEDULE_ALREADY_PAUSED');

    // ── resume ──
    const resumed = await request(app.getHttpServer())
      .post(`/api/v1/transactions/${parent.id}/schedule/resume`)
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(200);
    ...
    const resumeAgain = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(409);
    expect(resumeAgain.body.errorCode).toBe('TRANSACTION_SCHEDULE_NOT_PAUSED');

    // ── cancel ──
    const cancelled = await request(app.getHttpServer())
      .post(`/api/v1/transactions/${parent.id}/schedule/cancel`)
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(200);
    ...
    const cancelAgain = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(409);
    expect(cancelAgain.body.errorCode).toBe('TRANSACTION_SCHEDULE_ALREADY_CANCELLED');
    ...
    const pauseCancelled = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(409);
    expect(pauseCancelled.body.errorCode).toBe('TRANSACTION_SCHEDULE_CANCELLED');
    ...
    const resumeCancelled = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(409);
    expect(resumeCancelled.body.errorCode).toBe('TRANSACTION_SCHEDULE_CANCELLED');
  }, 60_000);
});
