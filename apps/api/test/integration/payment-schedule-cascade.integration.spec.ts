 ...
 *   - Parent payment edited RECURRING → ONE_TIME (cascade tear-down)
 *   - Parent payment hard-deleted via DELETE ?scope=all (cascade tear-down
 *     before the FK cascade)
 *
 * Boots Redis via testcontainers; PAYMENT_SCHEDULE_MIN_INTERVAL_MS=100 so
 * we can use sub-minute intervals.
 */
describe('PaymentSchedule cascade (integration)', () => {
  ...
  it('cascade on parent RECURRING → ONE_TIME tears down schedule + scheduler + audit reason=parent_type_changed', async () => {
    ...
    await request(app.getHttpServer())
      .patch(`/api/v1/payments/${parent.id}`)
      .set('Authorization', `Bearer ${user.accessToken}`)
      .send({ type: 'ONE_TIME' })
      .expect(200);
    ...
    const audits = await prisma.auditLog.findMany({
      where: { entity: 'PaymentSchedule', action: 'PAYMENT_SCHEDULE_DELETED', entityId: parent.id },
  ...
  it('cascade on parent DELETE ?scope=all tears down schedule + scheduler before parent.delete', async () => {
    ...
    await request(app.getHttpServer())
      .delete(`/api/v1/payments/${parent.id}?scope=all`)
      .set('Authorization', `Bearer ${user.accessToken}`)
      .expect(200);

    // Both row and parent gone.
    const row = await prisma.paymentSchedule.findFirst({ where: { id: scheduleId } });
    expect(row).toBeNull();
    const payment = await prisma.payment.findUnique({ where: { id: parent.id } });
