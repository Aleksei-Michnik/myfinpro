 ...
 * the full flow:
 *
 *  - Register a user, create a RECURRING payment with the existing
 *    POST /payments path.
 ...
 */
describe('PaymentSchedule CRUD (integration)', () => {
  ...
  async function createParentPayment(type: 'ONE_TIME' | 'RECURRING') {
    ...
    // the 6.17 RECURRING branch of POST /payments.
  ...
  it('POST → DB row + Redis scheduler key; DELETE removes both', async () => {
    const parent = await createParentPayment('RECURRING');
    ...
    const ours = schedulers.find((s) => s.key === `payment-schedule:${created.body.id}`);
    ...
    const rowAfter = await prisma.paymentSchedule.findUnique({ where: { paymentId: parent.id } });
    expect(rowAfter).toBeNull();

    const schedulersAfter = await queue.getJobSchedulers();
    expect(
      schedulersAfter.find((s) => s.key === `payment-schedule:${created.body.id}`),
    ).toBeUndefined();
  }, 60_000);

  it('rejects POST on a ONE_TIME parent with PAYMENT_SCHEDULE_PARENT_NOT_RECURRING (409)', async () => {
    const parent = await createParentPayment('ONE_TIME');
    const res = await request(app.getHttpServer())
      .post(`/api/v1/payments/${parent.id}/schedule`)
      .set('Authorization', `Bearer ${user.accessToken}`)
      .send({ everyMs: 60_000 })
      .expect(409);
    expect(res.body.errorCode).toBe('PAYMENT_SCHEDULE_PARENT_NOT_RECURRING');
  });
  ...
  it('PUT replaces the spec idempotently (same scheduler key, new opts)', async () => {
    ...
    await request(app.getHttpServer())
      .put(`/api/v1/payments/${parent.id}/schedule`)
      .set('Authorization', `Bearer ${user.accessToken}`)
      .send({ cron: '*/2 * * * *' })
      .expect(200);
    ...
    const schedulers = await queue.getJobSchedulers();
    const ours = schedulers.find((s) => s.key === `payment-schedule:${id}`);
    expect(ours).toBeDefined();
