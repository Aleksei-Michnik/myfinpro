...
/**
 * Phase 6.6 — GET /transactions integration tests.
 *
...
describe('GET /transactions (integration)', () => {
  ...
  /** Create a transaction directly via POST /transactions as the given user. */
  ...
  it('1. returns all personal transactions, newest first, with expected defaults', async () => {
    ...
    const { status, body } = await listTransactions(alice.accessToken);
  ...
  it('2. user B sees none of user A personal transactions', async () => {
    await createTransaction(alice.accessToken, basePayload());
    const { body } = await listTransactions(bob.accessToken);
    expect(body.data).toHaveLength(0);
  });

  // ── 3. Group visibility ──

  it('3. group member sees group transactions; non-member does not', async () => {
    await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );

    const forCarol = await listTransactions(carol.accessToken);
    expect(forCarol.body.data).toHaveLength(1);

    const forBob = await listTransactions(bob.accessToken);
  ...
  it('4. scope=personal returns only personal rows', async () => {
    await createTransaction(alice.accessToken, basePayload({ amountCents: 100 }));
    await createTransaction(
      alice.accessToken,
      basePayload({ amountCents: 200, attributions: [{ scope: 'group', groupId }] }),
    );

    const { body } = await listTransactions(alice.accessToken, { scope: 'personal' });
  ...
  it('5. scope=group:<id> as a member returns only group rows', async () => {
    ...
    const { body } = await listTransactions(alice.accessToken, { scope: `group:${groupId}` });
    expect(body.data).toHaveLength(1);
    expect(body.data[0].amountCents).toBe(200);
  });

  it('6. scope=group:<id> as non-member → 403 TRANSACTION_SCOPE_NOT_ACCESSIBLE', async () => {
    const { status, body } = await listTransactions(bob.accessToken, { scope: `group:${groupId}` });
    expect(status).toBe(403);
    expect(body.errorCode).toBe('TRANSACTION_SCOPE_NOT_ACCESSIBLE');
  });
  ...
  it('8. from/to date range filter', async () => {
    await createTransaction(alice.accessToken, basePayload({ occurredAt: '2026-02-15' }));
    await createTransaction(alice.accessToken, basePayload({ occurredAt: '2026-03-15' }));
    await createTransaction(alice.accessToken, basePayload({ occurredAt: '2026-04-15' }));

    const { body } = await listTransactions(alice.accessToken, {
  ...
  it('10. type=ONE_TIME filter (only type currently supported)', async () => {
    await createTransaction(alice.accessToken, basePayload());
    const { body } = await listTransactions(alice.accessToken, { type: 'ONE_TIME' });
    expect(body.data).toHaveLength(1);
    expect(body.data[0].type).toBe('ONE_TIME');
  });

  it('11. search substring-matches notes', async () => {
    await createTransaction(alice.accessToken, basePayload({ note: 'Lunch with Eve' }));
    await createTransaction(alice.accessToken, basePayload({ note: 'Groceries run' }));

    const { body } = await listTransactions(alice.accessToken, { search: 'lunch' });
    expect(body.data).toHaveLength(1);
  ...
  it('12. cursor pagination: 5 rows / limit=2 walks through three pages', async () => {
    for (let i = 0; i < 5; i++) {
      ...
      await createTransaction(
        alice.accessToken,
        basePayload({ amountCents: 100 + i, occurredAt: d }),
      );
    }

    // Page 1 — newest 2.
    const p1 = await listTransactions(alice.accessToken, { limit: 2 });
    expect(p1.body.data).toHaveLength(2);
    expect(p1.body.hasMore).toBe(true);
    expect(p1.body.nextCursor).not.toBeNull();

    // Page 2.
    const p2 = await listTransactions(alice.accessToken, { limit: 2, cursor: p1.body.nextCursor! });
    expect(p2.body.data).toHaveLength(2);
    expect(p2.body.hasMore).toBe(true);
    expect(p2.body.nextCursor).not.toBeNull();

    // Page 3 — last row.
    const p3 = await listTransactions(alice.accessToken, { limit: 2, cursor: p2.body.nextCursor! });
  ...
  it('13. invalid cursor → 400 TRANSACTION_INVALID_CURSOR', async () => {
    const { status, body } = await listTransactions(alice.accessToken, {
      cursor: 'definitely-not-base64url!',
    });
    expect(status).toBe(400);
    expect(body.errorCode).toBe('TRANSACTION_INVALID_CURSOR');
  });

  // ── 14. sort=amount_desc ──

  it('14. sort=amount_desc returns rows ordered by amountCents descending', async () => {
    await createTransaction(alice.accessToken, basePayload({ amountCents: 500 }));
    await createTransaction(alice.accessToken, basePayload({ amountCents: 2000 }));
    await createTransaction(alice.accessToken, basePayload({ amountCents: 900 }));

    const { body } = await listTransactions(alice.accessToken, { sort: 'amount_desc' });
    expect(body.data.map((r) => r.amountCents)).toEqual([2000, 900, 500]);
  });

  // ── 15-19. parentTransactionId / withParent filters + /occurrences alias (6.18.1.3) ──
   ...
   * `transaction-occurrence.integration.spec.ts` cover the happy-path pipeline
  ...
  it('15. /transactions/:id/occurrences returns children only (parent excluded)', async () => {
    const { parentId, childIds } = await seedParentWithChildren(alice.user.id, 3);

    const res = await request(app.getHttpServer())
      .get(`/api/v1/transactions/${parentId}/occurrences`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
  ...
  it('16. /transactions?parentTransactionId=<id> returns the same set as the alias', async () => {
    const { parentId, childIds } = await seedParentWithChildren(alice.user.id, 2);

    const { body } = await listTransactions(alice.accessToken, { parentTransactionId: parentId });
    const ids = body.data.map((r) => r.id as string);
    expect(new Set(ids)).toEqual(new Set(childIds));
  });

  it('17. /transactions/:id/occurrences for a caller who cannot see the parent → 404 (no leak)', async () => {
  ...
  it('18. withParent=true returns parents only; withParent=false returns occurrences only', async () => {
    ...
    await createTransaction(alice.accessToken, basePayload({ amountCents: 999 }));

    const onlyParents = await listTransactions(alice.accessToken, { withParent: 'true' });
    const parentTypes = onlyParents.body.data.map((r) => r.parentTransactionId);
    expect(parentTypes.every((p) => p === null)).toBe(true);
    expect(onlyParents.body.data).toHaveLength(2); // RECURRING parent + ONE_TIME

    const onlyChildren = await listTransactions(alice.accessToken, { withParent: 'false' });
    expect(onlyChildren.body.data).toHaveLength(2);
    expect(onlyChildren.body.data.every((r) => r.parentTransactionId !== null)).toBe(true);
  });

  it('19. /transactions/:id/occurrences for a one-time transaction returns an empty list (200, not 404)', async () => {
    const created = await createTransaction(alice.accessToken, basePayload());

    const res = await request(app.getHttpServer())
      .get(`/api/v1/transactions/${created.id}/occurrences`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
