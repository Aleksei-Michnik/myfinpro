...
/**
 * Phase 6.9 — POST /transactions/:id/star integration tests.
 *
 * Covers per-user toggle behaviour, multi-user star count, the 6.6 list
 * `starred=true|false` filter, the `starredByMe` field on GET /transactions/:id,
 * cascade on transaction delete, and the audit log trail.
 */
describe('POST /transactions/:id/star (integration)', () => {
  ...
  const toggleStar = async (
    ...
    const res = await request(app.getHttpServer())
      .post(`/api/v1/transactions/${transactionId}/star`)
      .set('Authorization', `Bearer ${token}`);
  ...
  it('1. first toggle returns { starred: true, starCount: 1 }', async () => {
    const { id } = await createTransaction(alice.accessToken, basePayload());
  ...
  it('2. second toggle by same user returns { starred: false, starCount: 0 }', async () => {
    ...
    const rows = await prisma.transactionStar.findMany({ where: { transactionId: id } });
    expect(rows).toHaveLength(0);
  });

  // ── 3. Two users star the same transaction ──

  it('3. two users star the same group transaction → starCount=2 after second call', async () => {
  ...
  it('4. GET /transactions/:id reflects per-user starredByMe after starring', async () => {
    const { id } = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );
  ...
  // ── 5. Non-visible transaction 404 ──

  it("5. user without visibility toggling another user's personal transaction → 404", async () => {
    const { id } = await createTransaction(alice.accessToken, basePayload());
  ...
  // ── 6. Group transaction: creator + member each star ──

  it('6. creator A and member C each star a group transaction → both see starredByMe=true; starCount=2', async () => {
    const { id } = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );
  ...
  it('7. GET /transactions?starred=true returns only the caller-starred transaction', async () => {
    const p1 = await createTransaction(alice.accessToken, basePayload({ amountCents: 100 }));
    await createTransaction(alice.accessToken, basePayload({ amountCents: 200 }));
    await toggleStar(alice.accessToken, p1.id);

    const starredRes = await request(app.getHttpServer())
      .get('/api/v1/transactions?starred=true')
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
    ...
    const unstarredRes = await request(app.getHttpServer())
      .get('/api/v1/transactions?starred=false')
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
  ...
  it('8. transaction delete cascades and removes TransactionStar rows', async () => {
    const { id } = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );
    ...
    const before = await prisma.transactionStar.findMany({ where: { transactionId: id } });
    expect(before).toHaveLength(2);

    // Hard-delete via DELETE ?scope=all (creator removes the only group attribution).
    await request(app.getHttpServer())
      .delete(`/api/v1/transactions/${id}?scope=all`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);

    const after = await prisma.transactionStar.findMany({ where: { transactionId: id } });
    expect(after).toHaveLength(0);
  });

  // ── 9. Audit log trail ──

  it('9. toggle on + off writes TRANSACTION_STARRED then TRANSACTION_UNSTARRED audit rows', async () => {
    const { id } = await createTransaction(alice.accessToken, basePayload());
    await toggleStar(alice.accessToken, id);
    await toggleStar(alice.accessToken, id);
    ...
    const audits = await prisma.auditLog.findMany({
      where: {
        entity: 'Transaction',
        entityId: id,
        action: { in: ['TRANSACTION_STARRED', 'TRANSACTION_UNSTARRED'] },
        userId: alice.user.id,
      },
      orderBy: { createdAt: 'asc' },
    });
    const actions = audits.map((a) => a.action);
    expect(actions).toEqual(['TRANSACTION_STARRED', 'TRANSACTION_UNSTARRED']);
  });
});
