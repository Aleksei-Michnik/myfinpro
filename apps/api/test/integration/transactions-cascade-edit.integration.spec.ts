...
/**
 * Phase 6 · Iteration 6.18.1.5 — PATCH /transactions/:id?propagate=... cascade edit.
 *
 ...
 */
describe('PATCH /transactions/:id (cascade edit, integration)', () => {
  ...
  const patch = async (
    ...
    const res = await request(app.getHttpServer())
      .patch(`/api/v1/transactions/${id}?propagate=${propagate}`)
  ...
  it('propagate=all updates the parent + every controllable child; out-of-scope-group child is skipped', async () => {
    ...
    expect(body.transaction.id).toBe(parentId);
    expect(body.transaction.amountCents).toBe(9999);
  ...
  it('propagate=self updates only the parent (children untouched)', async () => {
    ...
    expect(status).toBe(200);
    expect(body.transaction.id).toBe(parentId);
    expect(body.transaction.amountCents).toBe(4242);
  ...
  it('rejects an invalid propagate value with 400', async () => {
    ...
    const res = await request(app.getHttpServer())
      .patch(`/api/v1/transactions/${parentId}?propagate=everything`)
