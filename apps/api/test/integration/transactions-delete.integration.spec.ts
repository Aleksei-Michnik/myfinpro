...
/**
 * Phase 6.8 — DELETE /transactions/:id + PATCH /transactions/:id with attributions.
 *
 ...
 * rows (TRANSACTION_ATTRIBUTION_REMOVED / TRANSACTION_DELETED).
 */
describe('DELETE + PATCH(attributions) /transactions/:id (integration)', () => {
  ...
  const del = async (token: string, id: string, scope?: string) => {
    ...
    const res = await request(app.getHttpServer())
      .delete(`/api/v1/transactions/${id}${q}`)
      .set('Authorization', `Bearer ${token}`);
  ...
  afterEach(async () => {
    await prisma.auditLog.deleteMany({ where: { entity: 'Transaction' } });
    await prisma.transactionStar.deleteMany({});
  ...
  // 1. DELETE ?scope=personal on personal-only transaction → transaction deleted.
  it('1. DELETE ?scope=personal on personal-only transaction → transactionDeleted=true', async () => {
    const c = await createTransaction(alice.accessToken, basePayload());
    const { status, body } = await del(alice.accessToken, c.id, 'personal');
    expect(status).toBe(200);
    expect(body.transactionDeleted).toBe(true);
    expect(body.deletedAttributions).toBe(1);
    const row = await prisma.transaction.findUnique({ where: { id: c.id } });
    expect(row).toBeNull();
  });

  // 2. DELETE ?scope=personal on mixed → only personal removed; transaction remains.
  it('2. DELETE ?scope=personal on mixed transaction → only personal removed', async () => {
    ...
    const { status, body } = await del(alice.accessToken, c.id, 'personal');
    expect(status).toBe(200);
    expect(body.transactionDeleted).toBe(false);
    expect(body.deletedAttributions).toBe(1);
    // GET still works.
  ...
  it('4. DELETE ?scope=group:<id> as non-member → 404', async () => {
    ...
    expect(body.errorCode).toBe('TRANSACTION_NOT_FOUND');
  });

  // 5. DELETE ?scope=all on personal+group → both removed, transaction deleted.
  it('5. DELETE ?scope=all on a mixed transaction removes everything → transactionDeleted', async () => {
    const c = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'personal' }, { scope: 'group', groupId }] }),
    );
  ...
  //    Alice creates a group transaction where carol also has a personal attribution
  //    (edge case: typically the creator's personal attribution is theirs, so
  //    to simulate "another user's personal on the same transaction" we insert it
  ...
  it("6. DELETE ?scope=all keeps other users' accessible attributions", async () => {
    ...
    await prisma.transactionAttribution.create({
      data: {
        transactionId: c.id,
        scopeType: 'personal',
    ...
    // (both accessible). carol's personal survives, so transaction survives.
    const { status, body } = await del(alice.accessToken, c.id, 'all');
    expect(status).toBe(200);
    expect(body.transactionDeleted).toBe(false);
    expect(body.deletedAttributions).toBe(2);
    // Carol can still see the transaction.
    const g = await getOne(carol.accessToken, c.id);
    expect(g.status).toBe(200);
  });

  // 7. Implicit scope on ambiguous → 409 TRANSACTION_SCOPE_AMBIGUOUS.
  it('7. DELETE without scope on ambiguous → 409 TRANSACTION_SCOPE_AMBIGUOUS', async () => {
    const c = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'personal' }, { scope: 'group', groupId }] }),
    );
    ...
    expect(body.errorCode).toBe('TRANSACTION_SCOPE_AMBIGUOUS');
  ...
  it('8. DELETE ?scope=personal when user has no personal → 409 TRANSACTION_SCOPE_NOT_ATTRIBUTED', async () => {
    const c = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );
    // Alice has only a group attribution on this transaction.
    const { status, body } = await del(alice.accessToken, c.id, 'personal');
    expect(status).toBe(409);
    expect(body.errorCode).toBe('TRANSACTION_SCOPE_NOT_ATTRIBUTED');
  });

  // 9. PATCH attributions=[] on personal-only → 204, transaction hard-deleted.
  it('9. PATCH attributions=[] on personal-only → 204 + GET 404', async () => {
    const c = await createTransaction(alice.accessToken, basePayload());
    const res = await request(app.getHttpServer())
      .patch(`/api/v1/transactions/${c.id}`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .send({ attributions: [] });
  ...
  it('11. PATCH with non-member group in desired → 403 TRANSACTION_ATTRIBUTION_OUT_OF_SCOPE', async () => {
    const c = await createTransaction(alice.accessToken, basePayload());
    ...
    expect(body.errorCode).toBe('TRANSACTION_ATTRIBUTION_OUT_OF_SCOPE');
  });

  // 12. Non-creator member PATCH with attributions only → 403 TRANSACTION_NOT_OWNER.
  it('12. non-creator (group member) PATCH attributions → 403 TRANSACTION_NOT_OWNER', async () => {
    const c = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );
    ...
    expect(status).toBe(403);
    expect(body.errorCode).toBe('TRANSACTION_NOT_OWNER');
  });

  // 13. Audit rows on a full DELETE.
  it('13. full DELETE writes TRANSACTION_ATTRIBUTION_REMOVED (N) + TRANSACTION_DELETED audit rows', async () => {
    const c = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'personal' }, { scope: 'group', groupId }] }),
    );
    ...
    const audits = await prisma.auditLog.findMany({
      where: { entity: 'Transaction', entityId: c.id },
    });
  ...
  it('14. DELETE cascades to transaction_stars and transaction_comments', async () => {
    const c = await createTransaction(alice.accessToken, basePayload());
    await prisma.transactionStar.create({ data: { transactionId: c.id, userId: alice.user.id } });
    await prisma.transactionComment.create({
      data: { transactionId: c.id, userId: alice.user.id, content: 'will die' },
    });
    ...
    const stars = await prisma.transactionStar.findMany({ where: { transactionId: c.id } });
    const comments = await prisma.transactionComment.findMany({ where: { transactionId: c.id } });
