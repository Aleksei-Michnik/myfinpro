...
/**
 * Phase 6.7 — GET /transactions/:id + PATCH /transactions/:id integration tests.
 *
 ...
 * empty-string note → NULL, and best-effort TRANSACTION_UPDATED audit log.
 */
describe('GET + PATCH /transactions/:id (integration)', () => {
  ...
  it('1. creator fetches own personal transaction → 200 with matching body', async () => {
    const created = await createTransaction(alice.accessToken, basePayload({ note: 'dinner' }));
  ...
  it('2. non-visible user → 404 TRANSACTION_NOT_FOUND', async () => {
  ...
  it('3. group member fetches a group-attributed transaction → 200', async () => {
  ...
  it('4. creator PATCH {note} → 200 and DB reflects the change', async () => {
    const created = await createTransaction(alice.accessToken, basePayload({ note: 'before' }));
  ...
  it('5. non-creator member PATCH → 403 TRANSACTION_NOT_OWNER', async () => {
    const created = await createTransaction(
      alice.accessToken,
      basePayload({ attributions: [{ scope: 'group', groupId }] }),
    );
  ...
  it('6. non-member PATCH → 404 TRANSACTION_NOT_FOUND (404 masks 403)', async () => {
    const created = await createTransaction(alice.accessToken, basePayload());
  ...
  it('8. PATCH {direction: "IN"} on OUT category → 400 TRANSACTION_CATEGORY_DIRECTION_MISMATCH', async () => {
    const created = await createTransaction(alice.accessToken, basePayload());
  ...
  it('10. PATCH {occurredAt: 10d ahead} → 400 TRANSACTION_INVALID_DATE', async () => {
    const created = await createTransaction(alice.accessToken, basePayload());
    ...
    expect(status).toBe(400);
    expect(body.errorCode).toBe('TRANSACTION_INVALID_DATE');
  });

  // 11. Empty body → no-op, no audit
  it('11. PATCH {} → 200, no TRANSACTION_UPDATED audit row written', async () => {
    const created = await createTransaction(alice.accessToken, basePayload());
    const before = await prisma.auditLog.findMany({
      where: { action: 'TRANSACTION_UPDATED', entityId: created.id },
    });
  ...
  it('13. audit row TRANSACTION_UPDATED carries details.changed with the mutated keys', async () => {
    const created = await createTransaction(alice.accessToken, basePayload());
