 ...
 *   - POST /payments with type=INSTALLMENT (0%, 12×) pre-generates 12 PENDING
 *     children of exactly $100 (design acceptance).
 *   - POST /payments with type=LOAN (5%, 60×) matches the spreadsheet
 *     reference ($188.71 annuity).
 *   - GET /payments/:id/plan returns the joined amortisation table.
 *   - DELETE /payments/:id/plan cancels terminally (children flip to
 *     CANCELLED, repeat delete 409s, non-creator 404s).
 */
describe('POST /payments + /payments/:id/plan (plans, integration)', () => {
  ...
  it('INSTALLMENT $1200 / 12 @ 0% → parent + 12 PENDING children of exactly $100', async () => {
    ...
    const children = await prisma.payment.findMany({
      where: { parentPaymentId: created.id },
    ...
    const attrs = await prisma.paymentAttribution.findMany({
      where: { paymentId: children[0].id },
    });
    ...
    const plan = await prisma.paymentPlan.findUnique({ where: { paymentId: created.id } });
    expect(plan).toEqual(
  ...
  it('LOAN $10,000 / 60 @ 5% → french annuity matching the spreadsheet reference', async () => {
    ...
    const res = await request(app.getHttpServer())
      .get(`/api/v1/payments/${created.id}/plan`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(200);
  ...
  it('plan body on a non-plan type → 400 PAYMENT_PLAN_NOT_SUPPORTED; plan kind without body → 400 PAYMENT_PLAN_REQUIRED', async () => {
    const notSupported = await request(app.getHttpServer())
      .post('/api/v1/payments')
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .send(planBody({ type: 'ONE_TIME' }))
      .expect(400);
    expect(notSupported.body.errorCode).toBe('PAYMENT_PLAN_NOT_SUPPORTED');
    ...
    const required = await request(app.getHttpServer())
      ...
      .expect(400);
    expect(required.body.errorCode).toBe('PAYMENT_PLAN_REQUIRED');
  });
  ...
  it('DELETE cancels terminally: children flip to CANCELLED, repeat 409s, non-creator 404s', async () => {
    ...
    await request(app.getHttpServer())
      .delete(`/api/v1/payments/${created.id}/plan`)
  ...
  it('GET requires visibility: strangers 404, plan-less payments 404', async () => {
    const created = await createPlan();
    ...
    const res = await request(app.getHttpServer())
      .get(`/api/v1/payments/${oneTime.body.id}/plan`)
      .set('Authorization', `Bearer ${alice.accessToken}`)
      .expect(404);
    expect(res.body.errorCode).toBe('PAYMENT_PLAN_NOT_FOUND');
  });
});
