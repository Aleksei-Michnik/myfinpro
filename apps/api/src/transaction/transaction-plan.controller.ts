...
/**
 * Phase 6, iteration 6.19 — `TransactionPlan` REST surface.
 *
 * 1:1 with the parent transaction, hence the singular `/plan` path (mirrors the
 * 6.17.2 schedule sub-resource). Creation is INLINE on `POST /transactions`
 ...
 */
@ApiTags('Transaction Plans')
@Controller('transactions/:transactionId/plan')
export class TransactionPlanController {
  constructor(private readonly service: TransactionPlanService) {}
  ...
  @ApiOperation({
    ...
    description:
      ...
      'from the persisted parameters; each row joins its pre-generated child Transaction ' +
      '(occurrenceId + status) so clients can render per-row state. Visible to any user with ' +
      'access to the parent transaction.',
  })
  @ApiOkResponse({ description: 'Plan with amortisation rows', type: PlanResponseDto })
  @ApiNotFoundResponse({ description: 'Transaction not visible or has no plan' })
  ...
  @ApiOperation({
    ...
    description:
      ...
      '409 TRANSACTION_PLAN_ALREADY_CANCELLED on repeat.',
  })
  @ApiOkResponse({ description: 'Plan cancelled', type: PlanResponseDto })
  @ApiNotFoundResponse({ description: 'Transaction not owned by caller or has no plan' })
