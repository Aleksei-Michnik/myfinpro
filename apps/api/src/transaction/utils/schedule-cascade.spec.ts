import { buildSchedulerId, removeScheduleForTransaction } from './schedule-cascade';

describe('removeScheduleForTransaction', () => {
  let prisma: {
    transactionSchedule: { findUnique: jest.Mock; delete: jest.Mock };
  ...
  it('no-op (returns { removed: false }) when no schedule exists for the transaction', async () => {
    prisma.transactionSchedule.findUnique.mockResolvedValue(null);
    const result = await removeScheduleForTransaction(
      prisma as never,
      queue as never,
      'transaction-1',
    );
    expect(result).toEqual({ removed: false, scheduleId: null });
    expect(prisma.transactionSchedule.delete).not.toHaveBeenCalled();
  ...
  it('happy path: deletes the row, removes the scheduler key, writes the audit log', async () => {
    ...
    expect(prisma.transactionSchedule.delete).toHaveBeenCalledWith({ where: { id: 'sched-1' } });
  ...
  it('uses the supplied transaction client when opts.tx is set', async () => {
    const tx = {
      transactionSchedule: {
        findUnique: jest.fn().mockResolvedValue({ id: 'sched-2', transactionId: 'transaction-1' }),
        delete: jest.fn().mockResolvedValue({}),
      },
      auditLog: { create: jest.fn().mockResolvedValue({}) },
    };
    await removeScheduleForTransaction(prisma as never, queue as never, 'transaction-1', {
      tx: tx as never,
    });
    // Default prisma client must NOT have been touched.
    expect(prisma.transactionSchedule.findUnique).not.toHaveBeenCalled();
    expect(prisma.transactionSchedule.delete).not.toHaveBeenCalled();
    expect(prisma.auditLog.create).not.toHaveBeenCalled();
    // Tx client carried out the writes.
    expect(tx.transactionSchedule.findUnique).toHaveBeenCalled();
    expect(tx.transactionSchedule.delete).toHaveBeenCalledWith({ where: { id: 'sched-2' } });
  ...
  it('queue failure is swallowed (DB row already gone is the source of truth)', async () => {
    ...
    await expect(
      removeScheduleForTransaction(prisma as never, queue as never, 'transaction-1'),
...
describe('buildSchedulerId', () => {
  it('produces the deterministic key format', () => {
    expect(buildSchedulerId('abc')).toBe('transaction-schedule:abc');
  });
});
