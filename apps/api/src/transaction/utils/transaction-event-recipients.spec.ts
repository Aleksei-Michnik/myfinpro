...
import { computeTransactionRecipients } from './transaction-event-recipients';

describe('computeTransactionRecipients()', () => {
  ...
  it('always includes the creator', async () => {
    const r = await computeTransactionRecipients(makePrisma([]), [], 'creator-1');
    expect(r).toEqual(['creator-1']);
  });

  it('includes personal-attribution userIds', async () => {
    const r = await computeTransactionRecipients(
      makePrisma([]),
      [
  ...
  it('does not query memberships when no group attributions are present', async () => {
    ...
    await computeTransactionRecipients(
      prisma,
      [{ scopeType: 'personal', userId: 'creator-1', groupId: null }],
      'creator-1',
    );
