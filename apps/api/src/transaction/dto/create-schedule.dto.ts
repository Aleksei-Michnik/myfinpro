 ...
 * effective floor configurable via the `TRANSACTION_SCHEDULE_MIN_INTERVAL_MS`
 * env var (read by `TransactionScheduleService`) so integration tests can drop
 ...
 * `TransactionScheduleService` against `TRANSACTION_SCHEDULE_MIN_INTERVAL_MS`. We
...
/**
 * POST /transactions/:transactionId/schedule body. Iteration 6.17.2.
 *
 ...
 * check inside `TransactionScheduleService.dtoToRepeatOpts`).
 */
