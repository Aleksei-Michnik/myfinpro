...
/**
 * Inline plan body accepted by `POST /transactions` when
 * `type ∈ {INSTALLMENT, LOAN, MORTGAGE}` (Phase 6, iteration 6.19).
 *
 * The plan's principal is the transaction's own `amountCents` — there is
 * deliberately NO separate principal field, so the two can never diverge.
 * The plan kind is the transaction `type` itself, for the same reason.
 */
export class CreatePlanDto {
  ...
  @ApiProperty({
    ...
    example: 60,
    minimum: 1,
    maximum: PLAN_TRANSACTIONS_COUNT_MAX,
  })
  @IsInt()
  @Min(1)
  ...
  @ApiProperty({ enum: [...TRANSACTION_FREQUENCIES], example: 'MONTHLY' })
  @IsIn([...TRANSACTION_FREQUENCIES])
