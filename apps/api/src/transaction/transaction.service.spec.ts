...
describe('TransactionService', () => {
  ...
  /** Factory for the relation-loaded transaction returned by prisma.transaction.create(). */
  const makePersistedTransaction = (
    over: Partial<TransactionWithRelations> = {},
  ): TransactionWithRelations => ({
    id: 'pay-1',
    direction: 'OUT',
    type: 'ONE_TIME',
  ...
  const baseDto = (over: Partial<CreateTransactionDto> = {}): CreateTransactionDto => ({
    direction: 'OUT',
    type: 'ONE_TIME',
    amountCents: 1250,
    currency: 'USD',
  ...
  beforeEach(async () => {
    const mod: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getQueueToken(TRANSACTION_OCCURRENCES_QUEUE), useValue: queueMock },
        { provide: EventBus, useValue: eventBusMock },
      ],
    }).compile();
    service = mod.get(TransactionService);
    jest.clearAllMocks();
    prismaMock.auditLog.create.mockResolvedValue({});
    prismaMock.transactionSchedule.findUnique.mockResolvedValue(null);
    prismaMock.transactionSchedule.delete.mockResolvedValue({});
  ...
  describe('type guard', () => {
    ...
    it.each(['LIMITED_PERIOD'] as const)(
      'rejects type=%s with TRANSACTION_TYPE_NOT_IMPLEMENTED',
      async (t) => {
        ...
        try {
          ...
          expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_TYPE_NOT_IMPLEMENTED);
        }
      },
    );

    it.each(['INSTALLMENT', 'LOAN', 'MORTGAGE'] as const)(
      'rejects type=%s without a plan body (TRANSACTION_PLAN_REQUIRED, 6.19)',
      async (t) => {
        await expect(service.create('user-1', baseDto({ type: t }))).rejects.toThrow(
          BadRequestException,
        );
        try {
          ...
          expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_PLAN_REQUIRED);
        }
      },
    );

    it('accepts type=RECURRING and creates the transaction without auto-creating a schedule', async () => {
      categoryServiceMock.findById.mockResolvedValue(okCategory());
      prismaMock.transaction.create.mockResolvedValue(
        makePersistedTransaction({ type: 'RECURRING' }),
      );
      ...
      // — the create flow MUST NOT touch TransactionSchedule on its own.
      expect(prismaMock.transactionSchedule.findUnique).not.toHaveBeenCalled();
      expect(prismaMock.transactionSchedule.delete).not.toHaveBeenCalled();
      expect(queueMock.upsertJobScheduler).not.toHaveBeenCalled();

      await new Promise((res) => setImmediate(res));
      const auditArg = prismaMock.auditLog.create.mock.calls.find(
        (c) => (c[0] as { data: { action: string } }).data.action === 'TRANSACTION_CREATED',
  ...
  describe('schedule/plan guards', () => {
    it('rejects when schedule body is present', async () => {
      ...
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_SCHEDULE_NOT_SUPPORTED);
      }
    });
    ...
    it('rejects a plan body on a non-plan type (6.19: only INSTALLMENT/LOAN/MORTGAGE carry plans)', async () => {
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_PLAN_NOT_SUPPORTED);
      }
    });
  ...
  describe('currency', () => {
    it('rejects an unknown currency code', async () => {
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_INVALID_CURRENCY);
      }
    });
  });
  ...
  describe('date', () => {
    it('rejects occurredAt more than 1 day in the future', async () => {
      ...
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_INVALID_DATE);
      }
    });
  ...
  describe('category', () => {
    it('propagates failure from categoryService as TRANSACTION_INVALID_CATEGORY', async () => {
    ...
    it('rejects direction mismatch (OUT transaction with IN category)', async () => {
    ...
    it('accepts BOTH category for either transaction direction', async () => {
      categoryServiceMock.findById.mockResolvedValue(okCategory({ direction: 'BOTH' }));
      prismaMock.transaction.create.mockResolvedValue(
        makePersistedTransaction({ direction: 'IN' }),
      );
  ...
  describe('attributions', () => {
    ...
    it('rejects empty attributions', async () => {
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_NO_ATTRIBUTIONS);
      }
    });
    ...
    it('rejects personal + groupId (malformed)', async () => {
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_INVALID_ATTRIBUTION);
      }
    });
    ...
    it('rejects duplicate personal attributions', async () => {
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_DUPLICATE_ATTRIBUTION);
      }
    });
    ...
    it('rejects group attribution when user is not a member', async () => {
      ...
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_ATTRIBUTION_OUT_OF_SCOPE);
      }
    });
  });
  ...
  describe('create() success', () => {
    ...
    it('creates a personal-only transaction', async () => {
      prismaMock.transaction.create.mockResolvedValue(makePersistedTransaction());
    ...
    it('creates an INSTALLMENT parent with plan row + pre-generated PENDING children (6.19)', async () => {
      prismaMock.transaction.create
        .mockResolvedValueOnce(
          makePersistedTransaction({ id: 'pay-plan', type: 'INSTALLMENT', amountCents: 120_000 }),
        )
        .mockResolvedValue(makePersistedTransaction({ id: 'child' }));
      prismaMock.transactionPlan.create.mockResolvedValue({ id: 'plan-1' });
      ...
      // Plan row: principal is the transaction's own amountCents; the method
      // defaults by kind (INSTALLMENT -> equal).
      expect(prismaMock.transactionPlan.create).toHaveBeenCalledTimes(1);
      const planArg = (
        prismaMock.transactionPlan.create.mock.calls[0][0] as {
          data: Record<string, unknown>;
        }
      ).data;
      expect(planArg).toEqual(
        expect.objectContaining({
          transactionId: 'pay-plan',
          kind: 'INSTALLMENT',
      ...
      expect(prismaMock.transaction.create).toHaveBeenCalledTimes(13);
      const firstChild = (
        prismaMock.transaction.create.mock.calls[1][0] as {
          data: {
            type: string;
            status: string;
      ...
      const lastChild = (
        prismaMock.transaction.create.mock.calls[12][0] as {
    ...
    it('rejects an invalid plan body with TRANSACTION_PLAN_INVALID before any write (6.19)', async () => {
      try {
        await service.create(
          'user-1',
          baseDto({
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_PLAN_INVALID);
      }
      expect(prismaMock.$transaction).not.toHaveBeenCalled();
    });

    it('creates a group-only transaction when the caller is a member', async () => {
      prismaMock.groupMembership.findMany.mockResolvedValue([{ groupId: 'g1' }]);
      prismaMock.transaction.create.mockResolvedValue(
    ...
    it('creates a mixed personal + group transaction', async () => {
      prismaMock.groupMembership.findMany.mockResolvedValue([{ groupId: 'g1' }]);
      prismaMock.transaction.create.mockResolvedValue(
        makePersistedTransaction({
          attributions: [
    ...
    it('writes a TRANSACTION_CREATED audit log', async () => {
      prismaMock.transaction.create.mockResolvedValue(makePersistedTransaction());
      await service.create('user-1', baseDto());
    ...
    it('passes the note through to the transaction record', async () => {
      prismaMock.transaction.create.mockResolvedValue(
        makePersistedTransaction({ note: 'lunch money' }),
      );
    ...
    describe('list()', () => {
      ...
      const baseQ = (over: Partial<ListTransactionsQueryDto> = {}): ListTransactionsQueryDto =>
        ({ ...over }) as ListTransactionsQueryDto;

      const lastFindManyArg = () => {
        const calls = prismaMock.transaction.findMany.mock.calls;
      ...
      it('scope=group:<id> for a non-member: throws TRANSACTION_SCOPE_NOT_ACCESSIBLE', async () => {
        ...
        try {
          ...
          expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_SCOPE_NOT_ACCESSIBLE);
        }
      });
      ...
      it('cursor: malformed base64url returns 400 TRANSACTION_INVALID_CURSOR', async () => {
        ...
        try {
          ...
          expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_INVALID_CURSOR);
        }
      });

      it('cursor: wrong shape for sort returns 400 TRANSACTION_INVALID_CURSOR', async () => {
      ...
      it('starredByMe=false when stars include is empty', async () => {
        prismaMock.transaction.findMany.mockResolvedValue([makeRow({ stars: [] })]);
        const r = await service.list('user-1', baseQ({ limit: 10 }));
      ...
      // ── Iteration 6.18.1.3 — parentTransactionId / withParent filters ──

      describe('parentTransactionId / withParent filters (6.18.1.3)', () => {
        it('parentTransactionId narrows to children + asserts visibility on parent first', async () => {
          // assertVisible() goes through prisma.transaction.findFirst({ select: { id: true } }).
          prismaMock.transaction.findFirst.mockResolvedValue({ id: 'parent-1' });
          await service.list('user-1', baseQ({ parentTransactionId: 'parent-1' }));
          expect(lastFindManyArg().where.AND).toContainEqual({ parentTransactionId: 'parent-1' });
          // The first findFirst call IS the visibility probe.
          const visibilityArg = prismaMock.transaction.findFirst.mock.calls[0][0] as {
            where: { AND: Array<Record<string, unknown>> };
        ...
        it('parentTransactionId where caller cannot see the parent → 404 (no leak)', async () => {
          prismaMock.transaction.findFirst.mockResolvedValue(null);
          await expect(
            service.list('user-1', baseQ({ parentTransactionId: 'parent-1' })),
        ...
        it('withParent=true narrows to parents (parentTransactionId is null)', async () => {
          await service.list('user-1', baseQ({ withParent: 'true' }));
          expect(lastFindManyArg().where.AND).toContainEqual({ parentTransactionId: null });
        });

        it('withParent=false narrows to occurrences (parentTransactionId not null)', async () => {
        ...
        it('withParent omitted → no parent partition added (forward-compat)', async () => {
          ...
          const hasParentNull = andClauses.some(
            (c) => (c as { parentTransactionId?: unknown }).parentTransactionId === null,
          );
          const hasParentNotNull = andClauses.some(
            (c) =>
              typeof (c as { parentTransactionId?: { not?: unknown } }).parentTransactionId ===
                'object' &&
              (c as { parentTransactionId?: { not?: unknown } }).parentTransactionId?.not !==
                undefined,
          );
  ...
  describe('mapTransactionToSummary()', () => {
    it('produces ISO strings and the default zero/false counters', () => {
      const dto = mapTransactionToSummary(makePersistedTransaction(), { starredByMe: false });
    ...
    it('respects starredByMe=true and commentCount/hasDocuments overrides', () => {
      const dto = mapTransactionToSummary(makePersistedTransaction(), {
        starredByMe: true,
        commentCount: 3,
        hasDocuments: true,
      });
  ...
  describe('findByIdForUser()', () => {
    it("returns summary for the creator's own personal transaction", async () => {
      ...
      const arg = prismaMock.transaction.findFirst.mock.calls[0][0] as {
        where: { AND: Array<Record<string, unknown>> };
      };
    ...
    it('returns summary for a group member viewing a group transaction', async () => {
    ...
    it('exposes the source receipt back-link when the transaction came from a confirm (7.13)', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow({ receipt: { id: 'r-9' } }));
    ...
    it('throws TRANSACTION_NOT_FOUND when prisma returns null (non-member on group transaction)', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(null);
    ...
    it('throws TRANSACTION_NOT_FOUND for a completely unknown id', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(null);
      try {
  ...
  describe('update()', () => {
    beforeEach(() => {
      categoryServiceMock.findById.mockResolvedValue(okCategory());
      // The update() flow writes via transaction.update then reloads via findUnique.
      // Mirror the write in the subsequent findUnique so the summary reflects the edit.
      prismaMock.transaction.update.mockImplementation(async ({ data }: { data: unknown }) => {
        const merged = { ...makeFullRow(), ...(data as Record<string, unknown>) };
        prismaMock.transaction.findUnique.mockResolvedValue(merged);
        return merged;
      });
      // Default reload mirrors the row from findFirst.
      prismaMock.transaction.findUnique.mockImplementation(async () => {
        const last = prismaMock.transaction.findFirst.mock.results.slice(-1)[0];
    ...
    it('edits note only → single-field prisma update + audit with changed=["note"]', async () => {
      ...
      await new Promise((res) => setImmediate(res));
      const auditArg = prismaMock.auditLog.create.mock.calls.find(
        (c) => (c[0] as { data: { action: string } }).data.action === 'TRANSACTION_UPDATED',
    ...
    it('403 TRANSACTION_NOT_OWNER when caller is not the creator', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(
        makeFullRow({ createdById: 'user-other' }),
      );
      ...
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_NOT_OWNER);
      }
    });

    it('404 TRANSACTION_NOT_FOUND when the row is not visible', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(null);
      await expect(service.update('user-1', 'pay-1', { note: 'x' })).rejects.toThrow(
        NotFoundException,
      );
    ...
    it('400 TRANSACTION_CANNOT_EDIT_GENERATED_OCCURRENCE when parentTransactionId is set', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(
        makeFullRow({ parentTransactionId: 'parent-1' }),
      );
      try {
        await service.update('user-1', 'pay-1', { note: 'x' });
      } catch (err) {
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_CANNOT_EDIT_GENERATED_OCCURRENCE);
      }
    });

    it('400 TRANSACTION_CANNOT_EDIT_GENERATED_OCCURRENCE for type=RECURRING', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow({ type: 'RECURRING' }));
      try {
        await service.update('user-1', 'pay-1', { note: 'x' });
      } catch (err) {
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_CANNOT_EDIT_GENERATED_OCCURRENCE);
    ...
    it('direction change to IN with current OUT category → 400 TRANSACTION_CATEGORY_DIRECTION_MISMATCH', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow()); // OUT category
    ...
    it('category change to one not visible to user → 404 TRANSACTION_INVALID_CATEGORY', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow());
      categoryServiceMock.findById.mockRejectedValue(new NotFoundException('gone'));
      try {
    ...
    it('category change to a BOTH-direction category succeeds regardless of direction', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow({ direction: 'IN' }));
    ...
    it('future occurredAt (>1 day) → 400 TRANSACTION_INVALID_DATE', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow());
    ...
    it('note="" is coerced to null in the prisma data payload', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow({ note: 'before' }));
    ...
    it('direction change with no category change validates against existing category', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow()); // existing OUT category
      categoryServiceMock.findById.mockResolvedValue(okCategory({ direction: 'BOTH' }));
  ...
  describe('remove()', () => {
    it('explicit scope=personal: removes the personal attribution; transaction survives', async () => {
      ...
      prismaMock.groupMembership.findMany.mockResolvedValue([{ groupId: 'g1' }]);
      prismaMock.transactionAttribution.count.mockResolvedValue(1); // group remains
      // Second findUnique: reload after remove.
      prismaMock.transaction.findUnique.mockResolvedValueOnce(makeFullRow());

      const r = await service.remove('user-1', 'pay-1', { scope: 'personal' });
      expect(r.deletedAttributions).toBe(1);
      expect(r.transactionDeleted).toBe(false);
      expect(prismaMock.transactionAttribution.deleteMany).toHaveBeenCalledWith({
        where: { id: { in: ['attr-p'] } },
      });
    ...
    it('explicit scope=group:<id> as non-member: 404 TRANSACTION_NOT_FOUND (no visible attribution)', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(
        makeRawRow({
          createdById: 'user-other',
          attributions: [
    ...
    it('explicit scope=all: removes every accessible attribution; deletes transaction when none remain', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(
        makeRawRow({
          attributions: [
            {
      ...
      prismaMock.groupMembership.findMany.mockResolvedValue([{ groupId: 'g1' }]);
      prismaMock.transactionAttribution.count.mockResolvedValue(0); // nothing left

      const r = await service.remove('user-1', 'pay-1', { scope: 'all' });
      expect(r.deletedAttributions).toBe(2);
      expect(r.transactionDeleted).toBe(true);
      expect(r.transaction).toBeNull();
      expect(prismaMock.transaction.delete).toHaveBeenCalledWith({ where: { id: 'pay-1' } });
    });
    ...
    it("scope=all preserves other users' personal attributions", async () => {
      ...
      prismaMock.groupMembership.findMany.mockResolvedValue([]);
      prismaMock.transactionAttribution.count.mockResolvedValue(1); // other remains
      prismaMock.transaction.findUnique.mockResolvedValueOnce(makeFullRow());
    ...
    it('implicit scope: single accessible attribution → removes it', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(makeRawRow());
      prismaMock.groupMembership.findMany.mockResolvedValue([]);
      prismaMock.transactionAttribution.count.mockResolvedValue(0);
    ...
    it('implicit scope: multiple accessible → 409 TRANSACTION_SCOPE_AMBIGUOUS with accessibleScopes', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(
        makeRawRow({
          attributions: [
            {
      ...
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_SCOPE_AMBIGUOUS);
        const details = (
    ...
    it('scope=personal when user never had personal → 409 TRANSACTION_SCOPE_NOT_ATTRIBUTED', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(
        makeRawRow({
          attributions: [
            {
      ...
      try {
        ...
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_SCOPE_NOT_ATTRIBUTED);
      }
    });

    it('non-visible transaction (no id) → 404', async () => {
    ...
    it('fires TRANSACTION_ATTRIBUTION_REMOVED once per deletion + TRANSACTION_DELETED when transaction is hard-deleted', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(
        makeRawRow({
          attributions: [
            {
      ...
      expect(actions.filter((a) => a === 'TRANSACTION_ATTRIBUTION_REMOVED')).toHaveLength(2);
      expect(actions).toContain('TRANSACTION_DELETED');
    });

    it('does NOT fire TRANSACTION_DELETED when attributions still remain', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(
        makeRawRow({
          attributions: [
            {
      ...
      expect(actions).not.toContain('TRANSACTION_DELETED');
    });
  ...
  describe('update() with attributions', () => {
    ...
    const fullRowWithAttrs = (
      ...
      makeFullRow({
        attributions: attrs.map((a) => ({ ...a, group: a.group ?? null, transactionId: 'pay-1' })),
      });
    ...
    it('attributions identical to current accessible → no DB write, returns summary', async () => {
      ...
      expect(prismaMock.transactionAttribution.deleteMany).not.toHaveBeenCalled();
      expect(prismaMock.transactionAttribution.createMany).not.toHaveBeenCalled();
    });

    it('empty array removes all accessible → transaction deleted → returns null', async () => {
      const row = fullRowWithAttrs([
        { id: 'attr-p', scopeType: 'personal', userId: 'user-1', groupId: null },
      ]);
      prismaMock.transaction.findFirst.mockResolvedValue(row);
      ...
      expect(r).toBeNull();
      expect(prismaMock.transactionAttribution.deleteMany).toHaveBeenCalled();
      expect(prismaMock.transaction.delete).toHaveBeenCalledWith({ where: { id: 'pay-1' } });
    });
    ...
    it('adds a new group attribution for a member group', async () => {
      ...
      prismaMock.transaction.findFirst.mockResolvedValue(row);
      prismaMock.groupMembership.findMany.mockResolvedValue([{ groupId: 'g1' }]);
      prismaMock.transactionAttribution.count.mockResolvedValue(2);

      await service.update('user-1', 'pay-1', {
        attributions: [{ scope: 'personal' }, { scope: 'group', groupId: 'g1' }],
      });

      expect(prismaMock.transactionAttribution.createMany).toHaveBeenCalledWith({
        data: [{ transactionId: 'pay-1', scopeType: 'group', userId: null, groupId: 'g1' }],
      });
      expect(prismaMock.transactionAttribution.deleteMany).not.toHaveBeenCalled();
      await new Promise((res) => setImmediate(res));
      const actions = prismaMock.auditLog.create.mock.calls.map(
      ...
      expect(actions).toContain('TRANSACTION_ATTRIBUTION_ADDED');
    });
    ...
    it('replaces group g1 with g2 when user is a member of both', async () => {
      ...
      expect(prismaMock.transactionAttribution.createMany).toHaveBeenCalledWith({
        data: [{ transactionId: 'pay-1', scopeType: 'group', userId: null, groupId: 'g2' }],
      });
    });

    it('desired group the user is not a member of → 403 TRANSACTION_ATTRIBUTION_OUT_OF_SCOPE', async () => {
      const row = fullRowWithAttrs([
        { id: 'attr-p', scopeType: 'personal', userId: 'user-1', groupId: null },
      ]);
      prismaMock.transaction.findFirst.mockResolvedValue(row);
    ...
    it("desired attribution collides with another user's personal → 403 TRANSACTION_ATTRIBUTION_OUT_OF_SCOPE", async () => {
      const row = fullRowWithAttrs([
        // Caller's group is accessible, another user's personal is on the transaction but not accessible.
        { id: 'attr-g', scopeType: 'group', userId: null, groupId: 'g1' },
    ...
    it('duplicate in desired → 400 TRANSACTION_DUPLICATE_ATTRIBUTION', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow());
      prismaMock.groupMembership.findMany.mockResolvedValue([]);
      try {
        await service.update('user-1', 'pay-1', {
    ...
    it('malformed attribution → 400 TRANSACTION_INVALID_ATTRIBUTION', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow());
      prismaMock.groupMembership.findMany.mockResolvedValue([]);
      try {
        await service.update('user-1', 'pay-1', {
    ...
    it('non-creator with attributions in body → 403 TRANSACTION_NOT_OWNER', async () => {
      const row = fullRowWithAttrs([
        { id: 'attr-g', scopeType: 'group', userId: null, groupId: 'g1' },
      ]);
    ...
    it('attribution edit + scalar edit happen in one $transaction', async () => {
      ...
      expect(prismaMock.transactionAttribution.createMany).toHaveBeenCalled();
      await new Promise((res) => setImmediate(res));
      const actions = prismaMock.auditLog.create.mock.calls.map(
        (c) => (c[0] as { data: { action: string } }).data.action,
      );
      expect(actions).toContain('TRANSACTION_UPDATED');
      expect(actions).toContain('TRANSACTION_ATTRIBUTION_ADDED');
    });
  });
  ...
  describe('toggleStar()', () => {
    it('first toggle creates a TransactionStar row and returns starred=true', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue({ id: 'pay-1' });
      prismaMock.transactionStar.findUnique.mockResolvedValue(null);
      prismaMock.transactionStar.count.mockResolvedValue(1);

      const r = await service.toggleStar('user-1', 'pay-1');

      expect(r).toEqual({ starred: true, starCount: 1 });
      expect(prismaMock.transactionStar.create).toHaveBeenCalledWith({
        data: { transactionId: 'pay-1', userId: 'user-1' },
      });
      expect(prismaMock.transactionStar.delete).not.toHaveBeenCalled();
    });

    it('second toggle removes the existing row and returns starred=false', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue({ id: 'pay-1' });
      prismaMock.transactionStar.findUnique.mockResolvedValue({
        id: 'star-1',
        transactionId: 'pay-1',
        userId: 'user-1',
      });
      prismaMock.transactionStar.count.mockResolvedValue(0);

      const r = await service.toggleStar('user-1', 'pay-1');

      expect(r).toEqual({ starred: false, starCount: 0 });
      expect(prismaMock.transactionStar.delete).toHaveBeenCalledWith({ where: { id: 'star-1' } });
      expect(prismaMock.transactionStar.create).not.toHaveBeenCalled();
    });

    it('non-visible transaction returns 404 TRANSACTION_NOT_FOUND', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(null);
    ...
    it('different users see their own starred state; aggregated count reflects all', async () => {
      // Simulate user-2 starring a transaction that user-1 already starred.
      prismaMock.transaction.findFirst.mockResolvedValue({ id: 'pay-1' });
      prismaMock.transactionStar.findUnique.mockResolvedValue(null); // no row for user-2
      prismaMock.transactionStar.count.mockResolvedValue(2); // user-1 + user-2

      const r = await service.toggleStar('user-2', 'pay-1');

      expect(r).toEqual({ starred: true, starCount: 2 });
      expect(prismaMock.transactionStar.create).toHaveBeenCalledWith({
        data: { transactionId: 'pay-1', userId: 'user-2' },
      });
    });
    ...
    it('transaction throw propagates and no audit log is written', async () => {
      ...
      expect(actions).not.toContain('TRANSACTION_STARRED');
      expect(actions).not.toContain('TRANSACTION_UNSTARRED');
    });

    it('writes a TRANSACTION_STARRED audit log on the create-path', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue({ id: 'pay-1' });
      prismaMock.transactionStar.findUnique.mockResolvedValue(null);
      prismaMock.transactionStar.count.mockResolvedValue(1);
    ...
    it('writes a TRANSACTION_UNSTARRED audit log on the delete-path', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue({ id: 'pay-1' });
      prismaMock.transactionStar.findUnique.mockResolvedValue({
        id: 'star-1',
        transactionId: 'pay-1',
    ...
    it('starCount is read AFTER the transaction (separate transactionStar.count call)', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue({ id: 'pay-1' });
      prismaMock.transactionStar.findUnique.mockResolvedValue(null);
      prismaMock.transactionStar.count.mockResolvedValue(7);

      const r = await service.toggleStar('user-1', 'pay-1');

      expect(prismaMock.transactionStar.count).toHaveBeenCalledWith({
        where: { transactionId: 'pay-1' },
      });
      expect(r.starCount).toBe(7);
    });

    it('looks up the existing star via the transactionId_userId compound unique', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue({ id: 'pay-1' });
      prismaMock.transactionStar.findUnique.mockResolvedValue(null);
      prismaMock.transactionStar.count.mockResolvedValue(1);
      ...
      expect(prismaMock.transactionStar.findUnique).toHaveBeenCalledWith({
        where: { transactionId_userId: { transactionId: 'pay-1', userId: 'user-1' } },
      });
    });
  });
  ...
  describe('cascade: parent type-change & hard-delete', () => {
    ...
    it('update() RECURRING → ONE_TIME triggers cascade (schedule row + scheduler removed, audit reason=parent_type_changed)', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow({ type: 'RECURRING' }));
      prismaMock.transactionSchedule.findUnique.mockResolvedValue({
        id: 'sched-1',
        transactionId: 'pay-1',
      });
      prismaMock.transactionSchedule.delete.mockResolvedValue({ id: 'sched-1' });

      await service.update('user-1', 'pay-1', { type: 'ONE_TIME' });

      expect(prismaMock.transactionSchedule.delete).toHaveBeenCalledWith({
        where: { id: 'sched-1' },
      });
      expect(queueMock.removeJobScheduler).toHaveBeenCalledWith('transaction-schedule:sched-1');
      await new Promise((res) => setImmediate(res));
      const cascadeAudit = prismaMock.auditLog.create.mock.calls.find(
        (c) =>
          ...
          (c[0] as { data: { entity: string } }).data.entity === 'TransactionSchedule',
    ...
    it('update() ONE_TIME → RECURRING does NOT auto-create a schedule', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow({ type: 'ONE_TIME' }));
      await service.update('user-1', 'pay-1', { type: 'RECURRING' });
      expect(queueMock.upsertJobScheduler).not.toHaveBeenCalled();
      expect(prismaMock.transactionSchedule.findUnique).not.toHaveBeenCalled();
    });

    it('remove() final hard-delete with active schedule → cascade tears it down before transaction.delete', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(makeRawRow());
      prismaMock.groupMembership.findMany.mockResolvedValue([]);
      prismaMock.transactionAttribution.count.mockResolvedValue(0);
      prismaMock.transactionSchedule.findUnique.mockResolvedValue({
  ...
  describe('realtime emission', () => {
    ...
    it('create(): emits transaction.created with creator in userIds', async () => {
      prismaMock.transaction.create.mockResolvedValue(makePersistedTransaction());
      prismaMock.groupMembership.findMany.mockResolvedValue([]);

      await service.create('user-1', baseDto());

      const evt = emittedOf('transaction.created')[0] as
        | { userIds: string[]; transaction: { id: string } }
        | undefined;
      expect(evt).toBeDefined();
      expect(evt!.userIds).toContain('user-1');
      expect(evt!.transaction.id).toBe('pay-1');
    });
    ...
    it('create() with a group attribution: userIds includes every group member', async () => {
      ...
      const evt = emittedOf('transaction.created')[0] as { userIds: string[] };
      expect(new Set(evt.userIds)).toEqual(new Set(['user-1', 'm-1', 'm-2']));
    });

    it('update() scalar edit: emits transaction.updated', async () => {
      const row = makeFullRow();
      prismaMock.transaction.findFirst.mockResolvedValue(row);
      prismaMock.transaction.update.mockImplementation(async ({ data }: { data: unknown }) => {
      ...
      expect(emittedOf('transaction.updated')).toHaveLength(1);
    });

    it('update() empty body: does NOT emit transaction.updated', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(makeFullRow());
      await service.update('user-1', 'pay-1', {});
      expect(emittedOf('transaction.updated')).toHaveLength(0);
    });

    it('update() that empties accessible attributions: emits transaction.deleted, NOT transaction.updated', async () => {
      const row = makeFullRow();
      prismaMock.transaction.findFirst.mockResolvedValue(row);
      prismaMock.groupMembership.findMany.mockResolvedValue([]);
      prismaMock.transactionAttribution.count.mockResolvedValue(0);
      ...
      const r = await service.update('user-1', 'pay-1', { attributions: [] });
      expect(r).toBeNull();
      expect(emittedOf('transaction.deleted')).toHaveLength(1);
      expect(emittedOf('transaction.updated')).toHaveLength(0);
    });

    it('update() with attribution add: emits transaction_attribution.added + transaction.updated', async () => {
      const row = makeFullRow();
      prismaMock.transaction.findFirst.mockResolvedValue(row);
      prismaMock.groupMembership.findMany.mockResolvedValue([{ groupId: 'g1' }]);
      prismaMock.transactionAttribution.count.mockResolvedValue(2);
      ...
      const added = emittedOf('transaction_attribution.added');
    ...
    it('remove() final hard-delete: emits transaction.deleted only', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(makeRawRow());
      prismaMock.groupMembership.findMany.mockResolvedValue([]);
      prismaMock.transactionAttribution.count.mockResolvedValue(0);
      prismaMock.transactionSchedule.findUnique.mockResolvedValue(null);
      ...
      expect(emittedOf('transaction.deleted')).toHaveLength(1);
      expect(emittedOf('transaction_attribution.removed')).toHaveLength(0);
      expect(emittedOf('transaction.updated')).toHaveLength(0);
    });

    it('remove() partial scope: emits transaction_attribution.removed + transaction.updated, NOT transaction.deleted', async () => {
      prismaMock.transaction.findUnique.mockResolvedValueOnce(
        makeRawRow({
          attributions: [
            {
      ...
      expect(emittedOf('transaction.deleted')).toHaveLength(0);
      expect(emittedOf('transaction_attribution.removed')).toHaveLength(1);
      expect(emittedOf('transaction.updated')).toHaveLength(1);
    });
  });

  // ──────────────────────────────────────────────────────────────────────────
  // Iteration 6.18.1.5 — editTransactionWithPropagation() cascade edit
  // ──────────────────────────────────────────────────────────────────────────

  describe('editTransactionWithPropagation()', () => {
    ...
    /** Child occurrence row returned by transaction.findMany (attributions include). */
    ...
    beforeEach(() => {
      ...
      prismaMock.transactionAttribution.deleteMany.mockResolvedValue({ count: 0 });
      prismaMock.transactionAttribution.createMany.mockResolvedValue({ count: 0 });
    ...
    it('404 TRANSACTION_NOT_FOUND when the parent is not visible', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(null);
      try {
        await service.editTransactionWithPropagation('user-1', 'pay-1', { amountCents: 1 }, 'all');
      } catch (err) {
        expect(err).toBeInstanceOf(NotFoundException);
        expect(codeOf(err)).toBe(TRANSACTION_ERRORS.TRANSACTION_NOT_FOUND);
      }
    ...
    it('400 TRANSACTION_CANNOT_EDIT_GENERATED_OCCURRENCE for a child (parentTransactionId set)', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(
        recurringParent({ parentTransactionId: 'parent-x' }),
      );
      try {
        await service.editTransactionWithPropagation('user-1', 'pay-1', { amountCents: 1 }, 'all');
      } catch (err) {
    ...
    it('propagate=future: filters children by occurredAt >= now and updates them', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(recurringParent());
      prismaMock.transaction.findMany.mockResolvedValue([childRow({ id: 'child-future' })]);
      ...
      const findManyArg = prismaMock.transaction.findMany.mock.calls[0][0] as {
        where: { parentTransactionId: string; occurredAt?: { gte: Date } };
      };
      expect(findManyArg.where.parentTransactionId).toBe('pay-1');
    ...
    it('scope-guard: skips children attributed to a non-member group and reports the count', async () => {
      ...
      const updatedIds = prismaMock.transaction.update.mock.calls.map(
    ...
    it('preserves out-of-scope attributions on a target (never removes what the editor cannot control)', async () => {
      ...
      const deletedIds = prismaMock.transactionAttribution.deleteMany.mock.calls.flatMap(
    ...
    it('emits transaction.updated for the parent AND each updated child, after the transaction commits', async () => {
      ...
      await service.editTransactionWithPropagation('user-1', 'pay-1', { amountCents: 3000 }, 'all');

      const updated = emitted('transaction.updated');
      expect(updated).toHaveLength(3); // parent + 2 children
      const ids = updated.map((e) => (e.transaction as { id: string }).id);
    ...
    it('writes a TRANSACTION_UPDATED audit row per affected transaction with the propagate detail', async () => {
      prismaMock.transaction.findFirst.mockResolvedValue(recurringParent());
      prismaMock.transaction.findMany.mockResolvedValue([childRow({ id: 'c1' })]);

      await service.editTransactionWithPropagation('user-1', 'pay-1', { amountCents: 2222 }, 'all');
      await new Promise((res) => setImmediate(res));
      ...
      const updates = prismaMock.auditLog.create.mock.calls
        ...
        .filter((a) => a.data.action === 'TRANSACTION_UPDATED');
  ...
  describe('receipt-confirm helpers (7.9)', () => {
    ...
    describe('createExpenseWithinTx', () => {
      it('creates an OUT/ONE_TIME transaction with mapped attributions and a document', async () => {
    ...
    describe('publishCreated', () => {
      it('maps, computes recipients, and fans out transaction.created', async () => {
        const summary = await service.publishCreated(makePersistedTransaction());
        expect(eventBusMock.publish).toHaveBeenCalledWith(
          expect.objectContaining({ type: 'transaction.created', userIds: ['user-1'] }),
        );
