 ...
 * write.
 */
describe('TransactionOccurrenceProcessor', () => {
  ...
  function buildJob(overrides: Partial<Job<unknown>> = {}): Job<{
    ...
    return {
      data: { scheduleId: SCHEDULE_ID, transactionId: PARENT_ID, createdById: CREATOR_ID },
      processedOn: FIRED_MS,
      ...overrides,
    } as Job<{ scheduleId: string; transactionId: string; createdById: string }>;
  }
  ...
  function buildMocks(): Mocks {
    ...
    const findUniqueTransaction = jest.fn();
    ...
    const prisma = {
      transactionSchedule: { findUnique: findUniqueSchedule },
      transaction: { findUnique: findUniqueTransaction },
  ...
  function build(mocks: Mocks): TransactionOccurrenceProcessor {
    return new TransactionOccurrenceProcessor(
  ...
  it('happy path — creates child Transaction with parent shape, updates schedule, writes audit', async () => {
    ...
    const schedUpdate = (
      mocks.prisma as unknown as {
        _tx: { transactionSchedule: { update: jest.Mock } };
      }
    )._tx.transactionSchedule.update;
    ...
    expect(mocks.auditCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          action: 'TRANSACTION_OCCURRENCE_CREATED',
          entity: 'Transaction',
          entityId: PARENT_ID,
          userId: CREATOR_ID,
        }),
  ...
  it('orphan: schedule missing → removeJobScheduler called, no Transaction created', async () => {
    const mocks = buildMocks();
    (mocks.prisma.transactionSchedule as { findUnique: jest.Mock }).findUnique.mockResolvedValue(
      null,
    );
    ...
    expect(mocks.queue.removeJobScheduler).toHaveBeenCalledWith(
      `transaction-schedule:${SCHEDULE_ID}`,
    );
    expect(mocks.prisma.$transaction).not.toHaveBeenCalled();
  });

  it('parent type changed to ONE_TIME → removeJobScheduler called, no Transaction created', async () => {
    const mocks = buildMocks();
    (mocks.prisma.transactionSchedule as { findUnique: jest.Mock }).findUnique.mockResolvedValue(
      buildSchedule({ transaction: buildParent({ type: 'ONE_TIME' }) }),
    );
  ...
  it('idempotency: duplicate fire → fetches existing occurrenceId and skips insert', async () => {
    ...
    (mocks.prisma.transaction as { findUnique: jest.Mock }).findUnique.mockResolvedValue({
    ...
    expect((mocks.prisma.transaction as { findUnique: jest.Mock }).findUnique).toHaveBeenCalledWith(
      {
  ...
  it('cancelled takes precedence over parent checks (no removeJobScheduler call)', async () => {
    const mocks = buildMocks();
    (mocks.prisma.transactionSchedule as { findUnique: jest.Mock }).findUnique.mockResolvedValue(
      buildSchedule({ cancelledAt: new Date(), transaction: buildParent({ type: 'ONE_TIME' }) }),
    );
