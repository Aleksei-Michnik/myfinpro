...
export class AnthropicExtractionProvider implements ReceiptExtractionProvider {
  ...
  /**
   * Last-resort repair round-trip (see extraction-finalize.util.ts). Only the
   * rejected JSON and its errors travel — re-sending the document would
   * double the cost of the most expensive part of the request for a fix the
   * model can make from its own output.
   */
  private async repairCall(
    ...
    const response = await this.callModel([{ type: 'text', text: prompt }], undefined, 0);
    if (response.stop_reason === 'max_tokens') return null;
    return (
      response.content.find((block): block is Anthropic.TextBlock => block.type === 'text')?.text ??
      null
    );
  }
