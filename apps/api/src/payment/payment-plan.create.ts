import type { AmortizationMethod, PaymentPlanKind } from '@myfinpro/shared';
...
// functions (not methods on PaymentPlanService) so `PaymentService.create()`
// can call them without a PaymentService ⇄ PaymentPlanService dependency
// cycle: the plan service depends on PaymentService for `assertVisible`,
// while the payment service only needs this pure/tx-scoped logic.

/** Default amortisation method per plan kind (design §2.2). */
export function defaultPlanMethod(kind: PaymentPlanKind): AmortizationMethod {
 ...
 * Pure (no I/O) — `PaymentService.create()` runs this BEFORE opening the
...
export function validatePlanAndCompute(
  ...
  if (Number.isNaN(firstDueAt.getTime())) {
    throw new BadRequestException({
      ...
      errorCode: PAYMENT_ERRORS.PAYMENT_PLAN_INVALID,
    });
  }
  try {
...
/**
 * Create the `PaymentPlan` row + the N pre-generated child occurrences
...
export async function createPlanWithinTransaction(
  ...
  // clones insert per row — bounded by PLAN_PAYMENTS_COUNT_MAX and inside
  // one transaction.
