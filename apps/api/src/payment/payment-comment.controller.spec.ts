...
describe('PaymentCommentController', () => {
  let controller: PaymentCommentController;
  const svc = {
    list: jest.fn(),
    create: jest.fn(),
    update: jest.fn(),
  ...
  beforeEach(async () => {
    const mod: TestingModule = await Test.createTestingModule({
      controllers: [PaymentCommentController],
      providers: [{ provide: PaymentCommentService, useValue: svc }],
    }).compile();
    controller = mod.get(PaymentCommentController);
    jest.clearAllMocks();
  });

  it('1. GET / delegates to service.list with user.sub, paymentId, query', async () => {
  ...
  it('4. PATCH propagates 410 Gone from service', async () => {
    svc.update.mockRejectedValue(
      new HttpException({ errorCode: 'PAYMENT_COMMENT_DELETED' }, HttpStatus.GONE),
    );
