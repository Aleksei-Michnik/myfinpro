...
// realtime EventBus. A payment is visible to a user iff at least one of
// its attributions is personal to them OR targets a group they are a
// member of (mirrors `PaymentService.buildVisibilityWhere`). The creator
// is always included so that mutations they made — even on payments that
 ...
 * payment with the given attributions. The creator is always included.
 *
 ...
 */
export async function computePaymentRecipients(
