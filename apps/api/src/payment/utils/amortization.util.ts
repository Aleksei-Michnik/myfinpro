// Phase 6, iteration 6.19 — pure amortisation math for PaymentPlan.
//
...
//   - `french` — classic annuity: constant per-period payment
...
export function dueAtForIndex(firstDueAt: Date, frequency: PaymentFrequency, index: number): Date {
  const steps = index - 1;
  switch (frequency) {
    case 'DAILY':
...
export function calculateAmortization(input: AmortizationInput): AmortizationRow[] {
  const { principalCents, interestRate, paymentsCount, method, firstDueAt, frequency } = input;

  if (!Number.isInteger(principalCents) || principalCents <= 0) {
    throw new RangeError('principalCents must be a positive integer');
  }
  if (!Number.isInteger(paymentsCount) || paymentsCount <= 0) {
    throw new RangeError('paymentsCount must be a positive integer');
  }
  ...
  if (method === 'equal' || perPeriodRate === 0) {
    const base = Math.floor(principalCents / paymentsCount);
    const extraCents = principalCents - base * paymentsCount;
  ...
  // French annuity. Constant payment rounded once; per-row interest rounded
  // on the declining balance; final row absorbs the rounding drift.
  const r = perPeriodRate;
  const n = paymentsCount;
