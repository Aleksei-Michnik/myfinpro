...
export function ProductFormDialog({
  ...
  // The scanner sits above this dialog: while it is open, ESC and the backdrop
  // belong to it.
  const requestClose = () => {
    if (!scannerOpen) onCancel();
  };
  ...
  return (
    <Dialog
      ...
      testId="product-form-dialog"
      backdropTestId="product-form-backdrop"
      className="space-y-4"
            ...
            <div className="flex flex-wrap items-center gap-2" data-testid="product-pictures">
              {editing
                ? images.map((img, index) => (
                    <div key={img.id} className="relative" data-testid={`product-picture-${index}`}>
                      <img
