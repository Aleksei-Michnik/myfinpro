...
**Iteration count summary**: Phase 0 (8), Phase 1 (12), Phase 2 (4), Phase 3 (4), Phase 4 (15), Phase 5 (14), Phase 6 — unified Transaction Management (21, replaces original Phase 6 + 7), Phase 7 — Receipts & Extraction (13, incl. 7.11–7.13 follow-ups), Phase 8 — Product Catalog & Barcode (10), Phase 9 — Purchase Analytics (8), Phase 10 — Budgets (10), Phase 11 — MCP Server (8), Phase 12-Core (4), Phase 12-Transactions (12), Phase 13 — Mini App (10), Phase 14 — Bot Receipts (6), Phase 15 — Bot Analytics (4), Phase 16 — LLM Assistant (7), Phase 17 — WebMCP (4). **Total: 174 iterations.**
...
| Phase 6: Transaction Management (incomes + expenses, unified) | 21         | Large         | Replaces original Phase 6 (Income) + Phase 7 (Expense); single Transaction entity with direction IN/OUT; shared UI, attributions, categories, schedules, plans, stars, comments; BullMQ recurring worker; amortisation |
| Phase 7: Receipt Ingestion & LLM Extraction                   | 13         | Medium        | Pluggable vision-LLM extraction; receipts/items/merchants schema; review flow; URL ingestion; follow-ups: HEIC→JPEG, readable URL snapshots, transaction-first intake (receipt = transaction document)                 |
...
| Phase 10: Budgets                                             | 10         | Medium        | Renumbered from original Phase 8; depends on transactions                                                                                                                                                              |
...
| Phase 18: Centralized Search                                  | 7          | Medium        | One search over transactions/receipts/items/comments: merchants, products/services, hierarchical categories (many-to-many), amount/date filters + sorting, include/exclude terms; LLM enrichment at receipt intake     |
| Phase 19: LLM Usage & Cost Tracking                           | 6          | Small-Medium  | Per-operation token/cost ledger (BYOK-aware) per provider + model, linked to the source entity (receipt, future agents/MCP); usage dashboard                                                                           |
...
| Iteration | Objective               | Scope                                                                                                                                                                                                                        | Testing            | CI/CD                          | Deployment | Acceptance Criteria                                              |
| --------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------- | ---------------------------------------------------------------- |
| 6.1       | Shared types + DTOs     | `packages/shared`: `TransactionDirection`, `TransactionType`, `TransactionStatus`, `TransactionFrequency`, `AttributionScope`, default category slugs                                                                        | Unit tests         | lint + typecheck + unit        | N/A        | Types importable across workspaces                               |
| 6.2       | DB schema + migration   | Prisma models: `transactions`, `transaction_attributions`, `categories`, `transaction_schedules`, `transaction_plans`, `transaction_documents`, `transaction_comments`, `transaction_stars` + indexes; expand-only migration | Migration tests    | lint + typecheck + unit        | Deploy     | Migration applies on staging; schema matches design doc          |
...
#### Part B — Transaction core API (one-time)

| Iteration | Objective              | Scope                                                                                                                        | Testing            | CI/CD                                       | Deployment         | Acceptance Criteria                                                                      |
| --------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------- |
| 6.5       | Create transaction API | `POST /transactions` (ONE_TIME); validates attributions, category direction, currency, amount                                | Unit + integration | lint + typecheck + integration              | Deploy             | Personal, group, and mixed attributions all create successfully; invalid scopes rejected |
| 6.6       | List transactions API  | `GET /transactions` with cursor pagination and filters: scope / direction / category / date / starred / type / search / sort | Unit + integration | lint + typecheck + integration              | Deploy             | Filters honoured; pagination stable; visibility = user's personal ∪ member groups        |
| 6.7       | Get-one + edit API     | `GET /transactions/:id` (access guard) + `PATCH /transactions/:id` (creator only)                                            | Unit + integration | lint + typecheck + integration              | Deploy             | Creator edits; non-creator blocked; non-accessor 404                                     |
| 6.8       | Delete API (scoped)    | `DELETE /transactions/:id?scope=personal                                                                                     | group:<id>         | all`; last attribution triggers hard delete | Unit + integration | lint + typecheck + integration                                                           | Deploy | Only accessible attributions removed; other users' / non-member groups' attributions kept |

#### Part C — Social features API

| Iteration | Objective       | Scope                                                                                                                  | Testing            | CI/CD                          | Deployment | Acceptance Criteria                                                                 |
| --------- | --------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------- | ----------------------------------------------------------------------------------- |
| 6.9       | Star toggle API | `POST /transactions/:id/star` toggles per-user star; list returns `starredByMe`                                        | Unit + integration | lint + typecheck + integration | Deploy     | Star isolation per user; list filter `?starred=true` works                          |
| 6.10      | Comments API    | `GET/POST/PATCH/DELETE /transactions/:id/comments[/:commentId]` — any accessor can comment; author can edit/delete own | Unit + integration | lint + typecheck + integration | Deploy     | Comment thread functional; edit/delete limited to author; soft delete preserves row |

#### Part D — DRY frontend

| Iteration | Objective                    | Scope                                                                                                                                                    | Testing            | CI/CD                   | Deployment | Acceptance Criteria                                                                              |
| --------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ----------------------- | ---------- | ------------------------------------------------------------------------------------------------ |
| 6.11      | TransactionContext + helpers | `TransactionProvider`, types, formatters, `remember.ts` (localStorage for last-used scopes/direction/type)                                               | Unit (vitest)      | lint + typecheck + unit | Deploy     | Provider mounts; formatters handle USD/ILS/EUR × en/he                                           |
| 6.12      | `<TransactionsList>`         | Reusable list with filter bar, sorting, star/edit/delete controls; responsive table/card                                                                 | Unit + interaction | lint + typecheck + unit | Deploy     | Component reusable with any scope; filters trigger refetch; delete dialog presents scope options |
| 6.13      | `<TransactionFormDialog>`    | Add/edit dialog: direction toggle, amount+currency, note, scope multiselect (w/ remember), category picker, type selector with recurring/plan disclosure | Unit + interaction | lint + typecheck + unit | Deploy     | Valid payloads for ONE_TIME, RECURRING, INSTALLMENT, LOAN form submissions                       |
| 6.14      | Transaction detail page      | `/transactions/:id` with note, documents placeholder, comments thread, star, edit/delete; schedule/plan summary                                          | Unit + E2E         | lint + typecheck + unit | Deploy     | Detail page deep-links from list and dashboard; comments + star update live                      |
...
| Iteration | Objective                               | Scope                                                                                                                                                     | Testing    | CI/CD                          | Deployment | Acceptance Criteria                                                                       |
| --------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------------ | ---------- | ----------------------------------------------------------------------------------------- |
| 6.16      | Personal + group views, CategoryManager | `/transactions?scope=personal`, `/transactions/starred`, Transactions tab inside `/groups/[groupId]`, personal + group Category manager on settings pages | Unit + E2E | lint + typecheck + integration | Deploy     | All scope filters usable; categories can be added/edited/deleted from both settings pages |

#### Part F — Recurring & limited-period

| Iteration | Objective                     | Scope                                                                                   | Testing            | CI/CD                          | Deployment | Acceptance Criteria                                                                         |
| --------- | ----------------------------- | --------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------- | ------------------------------------------------------------------------------------------- |
| 6.17      | Schedules API + BullMQ worker | `TransactionSchedule` flow, hourly cron, catch-up for missed occurrences, audit logging | Unit + integration | lint + typecheck + integration | Deploy     | Recurring transaction with back-dated `startsAt` generates missed occurrences on first tick |
| 6.18      | Recurring UI                  | "Make recurring" disclosure in form, schedule summary on detail page, pause/cancel      | Unit + E2E         | lint + typecheck + integration | Deploy     | User can create / pause / cancel recurring transactions from the UI                         |

#### Part G — Installments & loans/mortgages

| Iteration | Objective                | Scope                                                                                            | Testing            | CI/CD                          | Deployment | Acceptance Criteria                                                             |
| --------- | ------------------------ | ------------------------------------------------------------------------------------------------ | ------------------ | ------------------------------ | ---------- | ------------------------------------------------------------------------------- |
| 6.19      | Plans API + amortisation | `TransactionPlan` creation, amortisation util (equal + french), pre-generate N occurrences       | Unit + integration | lint + typecheck + integration | Deploy     | Reference amortisation schedules match hand-calculated fixtures                 |
...
> - **One confirmed receipt = one `Transaction`** (direction `OUT`, type `ONE_TIME`) with the file attached as a `TransactionDocument` (kind `receipt`) and line items in a new `receipt_items` table.
> - **Per-item categories**: each line item is classified by the LLM against the user's _existing_ categories (system + personal + group, direction `OUT`); the user confirms during review. The transaction keeps a primary category; analytics uses item-level categories.
...
| 7.9       | Confirm → transaction      | Confirming creates `Transaction` (OUT) + attributions (remembered scopes) + `TransactionDocument` link; transaction detail page shows line-item breakdown              | Integration + E2E  | full suite                     | Deploy     | Confirmed receipt appears as a transaction with correct total, scope, document, and item breakdown      |
...
> **Follow-ups (2026-07-10)** — two recognition-quality defects found in staging verification, plus one UX/architecture correction:
>
> 1. **HEIC (7.11)**: the upload whitelist accepts `image/heic` (the iPhone camera default), but vision LLM APIs reject HEIC and browsers cannot render it in previews — so the platform's most likely mobile upload failed extraction _and_ preview. Fix: convert HEIC → JPEG once, at storage time (WASM decoder — no native image libs in the container); everything downstream (extraction, preview, `TransactionDocument`) sees a JPEG.
> 2. **URL snapshots (7.12)**: the fetcher handed raw HTML (markup, scripts, styles — mostly noise) to the extraction model, which measurably degraded recognition on online receipts. Fix: reduce the snapshot to readable text (drop script/style/head, tags → whitespace, decode entities, collapse) before the provider call.
> 3. **Transaction-first intake (7.13)**: receipts and transactions lived as parallel top-level objects, but a receipt is not a peer of a transaction — it is an **attribute of a transaction**: the document that proves it and carries its details. Primary intake moves into the transaction interface (**Add transaction → From receipt**), which uploads the file and hands off to the existing extract → review → confirm pipeline that ends in the transaction; the transaction detail page links back to its source receipt. `/receipts` remains as the pipeline/history view of in-flight documents.

| Iteration | Objective                        | Scope                                                                                                                                                                                                                       | Testing        | CI/CD                   | Deployment | Acceptance Criteria                                                                                                                  |
| --------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ----------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 7.11      | HEIC → JPEG at storage           | Convert `image/heic` uploads to JPEG in the storage service (WASM decode, quality ~0.9); stored artifact, preview, extraction input, and `TransactionDocument` all become JPEG; corrupt HEIC → structured 400               | Unit tests     | lint + typecheck + unit | Deploy     | An iPhone HEIC upload extracts and previews exactly like a JPEG upload                                                               |
| 7.12      | Readable URL snapshots           | `htmlToReceiptText()` reduction (strip script/style/head/comments, block tags → newlines, entity decode, whitespace collapse, capped) applied to HTML fetches before the provider call; non-HTML passes through             | Unit tests     | lint + typecheck + unit | Deploy     | Online receipt extraction receives clean text; noisy-page fixture reduces to its visible content                                     |
| 7.13      | Transaction-first receipt intake | "From receipt" action in the Add-transaction dialog: upload → redirect to review → confirm lands back on the transaction; transaction detail shows a link to its source receipt (`receiptId` on the detail DTO); i18n EN+HE | UI + API tests | lint + typecheck + unit | Deploy     | User starts from Add transaction, uploads, reviews, confirms; the resulting transaction links back to its receipt in both directions |
...
> **Follow-ups (2026-07-12/13)** — receipt intake & transaction integration; detailed design: `docs/phase-8-receipt-intake-design.md`.

| Iteration | Objective                     | Scope                                                                                                                                                                                         | Testing            | CI/CD                          | Deployment | Acceptance Criteria                                                                                     | Status  |
| --------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------- | ------------------------------------------------------------------------------------------------------- | ------- |
| 8.11      | Per-user LLM selection        | Curated Anthropic/OpenAI model catalog, per-user selection, encrypted BYOK keys (runbook §9), per-uploader provider resolution in the worker                                                  | Unit + integration | lint + typecheck + integration | Deploy     | User picks a model in Settings; extraction runs on it with the user's or the shared key                 | shipped |
| 8.12      | URL receipts by content       | URL responses routed by actual content: PDF/image → native vision inputs, HTML reduced-then-capped; unknown binary fails with a clear reason                                                  | Unit + live E2E    | lint + typecheck + unit        | Deploy     | An e-receipt link serving a PDF or image extracts as well as a direct upload                            | shipped |
| 8.13      | Intake chooser                | "From receipt" in Add Transaction becomes a chooser: upload from device / **add from URL**                                                                                                    | UI tests           | lint + typecheck + unit        | Deploy     | A URL pasted in the Add Transaction dialog lands in the extract → review → confirm pipeline             | shipped |
| 8.14      | Manual receipt by barcode     | Camera scanning composes a receipt: per scan → product lookup/create, quantity + unit price per line, price memory (same barcode → qty bump; history prefills price); `POST /receipts/manual` | UI + integration   | lint + typecheck + integration | Deploy     | A receipt can be composed by scanning products and lands in review with items pre-linked                | shipped |
| 8.15      | Attach + reconcile            | Attach receipt (file/URL) to an existing transaction; after extraction a reconciliation dialog offers keep/update per field (total, category); items/products update regardless               | UI + integration   | lint + typecheck + integration | Deploy     | Attaching a receipt to a transaction updates products always and the transaction header only on request | shipped |
| 8.16      | Receipt⇄transaction invariant | No receipt without a transaction: receipts directory mirrors transactions; pre-confirm receipts presented as transient intake; stale-draft cleanup                                            | Full test suite    | full suite                     | Deploy     | Every listed receipt links its transaction; orphan receipts no longer exist as a durable state          | planned |

> **Follow-ups (2026-07-17)** — mobile item cards, product pictures & media pipeline, extraction transparency; detailed design: `docs/phase-8-ux-followups-design.md`. (8.17–8.23 shipped under `docs/phase-8-receipt-intake-design.md`.)

| Iteration | Objective                   | Scope                                                                                                                                                                                                                                                                                    | Testing            | CI/CD                          | Deployment | Acceptance Criteria                                                                                                                                                                | Status  |
| --------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| 8.24      | Receipt items as cards      | Review-page item rows become one card per item (mobile-first, replaces the table at every width): name, qty, unit price, discount, line total, category, match chip, product thumbnail; `ProductThumb` extracted and reused                                                              | UI tests           | lint + typecheck + unit        | Deploy     | Every item field is visible and editable on a phone with no horizontal scrolling; desktop renders the same cards                                                                   | shipped |
| 8.25      | Product pictures + pipeline | Up to 5 pictures per product (`product_images`; `image_ref` retired via expand→contract); WebP+AVIF detail + thumbnail renditions, Accept-negotiated serving; shared capture control ("Add picture" in the product dialog); confirmed receipts' image pages re-encoded post-CONFIRM      | Unit + integration | lint + typecheck + integration | Deploy     | Pictures addable from the create dialog via picker or camera; AVIF served with WebP fallback; cards consume the thumb rendition; receipt storage shrinks with extraction untouched | shipped |
| 8.26      | Extraction progress UX      | Transient `receipt.extraction.progress` SSE event; staged live progress (send → think → generate → continue) with the selected model's name; Anthropic summarized-thinking stream (expandable), OpenAI streaming best-effort; animated indicator, reduced-motion safe; nothing persisted | Unit + UI tests    | lint + typecheck + unit        | Deploy     | While extracting, the review page shows live stages and the model's thought stream where available; providers without one degrade to animated generic states                       | shipped |

| 8.27 | Image handling standard | `docs/image-handling.md` conventions doc; shared `lib/upload.ts` accept lists + client-side validation; shared `ProductImage` placeholder; receipt lightbox generalized to `components/ui/DocumentViewer`; `ProductGallery` on the product detail page (view/add/remove/make-primary + lightbox); `ProductQuickViewDialog` opened from receipt item cards and transaction purchase details | Unit + UI tests | lint + typecheck + unit | Deploy | Product pictures browsable as a gallery with full-size zoom from the product page and from any product thumbnail on receipts/transactions; all upload surfaces validate type/size client-side with shared constants | shipped |
...
>
> **Detailed design**: See [`docs/phase-9-analytics-design.md`](docs/phase-9-analytics-design.md) — hybrid grain (item rows + header rows + balancing rows), per-currency aggregation, Recharts, nightly habit job.
...
> **Re-plan (2026-07-03)**: Renumbered from original Phase 8, content unchanged; moved after the receipt/product/analytics track. Only depends on Phase 6 (transactions), so it can run in parallel with Phases 7–9.
>
...
| 10.10     | Due transaction reminders | Reminder scheduling for upcoming transactions                          | Integration tests | lint + typecheck + integration | Deploy     | Reminders scheduled                                                                                  |
...
>
> **Capabilities**: read purchases/transactions/stats/habits for advice; add transactions; **upload receipts from chat** (the chat client becomes an alternative UI — our system plays the database role with add/read layers); confirm product matches conversationally; add/read **purchase comments** to remember item context.
...
| 11.3      | Read tools                 | Search/list transactions, receipts, purchase items, products; stats + habit-summary tools (reusing Phase 9 aggregations)        | Integration tests  | lint + typecheck + integration | Deploy     | LLM client can answer "what do I buy weekly?" and "how much did I spend on groceries?" via tools   |
| 11.4      | Write tools                | Add transaction; upload receipt (base64 / URL → Phase 7 pipeline); confirm/reject product match proposals conversationally      | Integration tests  | lint + typecheck + integration | Deploy     | Receipt uploaded from an LLM chat lands in the same review pipeline; matches confirmable via tools |
...
| 12.9      | Notifications         | Due transaction and low balance alerts via Telegram | Integration tests | lint + typecheck + integration | Deploy     | Alerts sent to linked users when thresholds triggered     |
...
| 14.4      | Quick confirm flow       | Inline confirmation of merchant/total/date for simple receipts; creates transaction        | E2E tests         | full suite                     | Deploy     | Simple receipt confirmable entirely inside Telegram                           |
...
| 17.2      | Read tools       | Expose search transactions/products, stats, and current-view context as WebMCP tools               | Unit + manual (Chromium) | lint + typecheck + unit | Deploy     | In-browser agent can query purchases and stats without DOM automation                     |
| 17.3      | Write tools      | Add transaction/purchase, start receipt upload, confirm product matches                            | Unit + manual (Chromium) | lint + typecheck + unit | Deploy     | In-browser agent can add a purchase end-to-end; all writes go through existing API guards |
...
### Phase 18: Centralized Search

> **Added (2026-07-15)**: by user request. One search box that finds transactions, receipts, receipt items and comments by anything the user remembers — the shop name, a date, a product or service ("bread", "hairdresser"), a category at any depth ("drinks" ⊃ "juice"), or an amount — with include/exclude terms, sorting and filtering. Requires a deeper categorisation model than the current single `categoryId` per row: hierarchical categories with **many-to-many** attribution (a purchase can be both "food" and "gift"). Receipt intake gains an LLM enrichment step that attributes new receipts/items into that taxonomy so search hits are rich from day one. Complements Phase 9 (analytics aggregates); search is the cross-entity _finder_.

| Iteration | Objective          | Scope                                                                                                                                              | Testing            | CI/CD                          | Deployment       | Acceptance Criteria                                                                                          |
| --------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------------- | ------------------------------------------------------------------------------------------------------------ |
| 18.1      | Category taxonomy  | Hierarchical categories (parent chains) + many-to-many entity↔category attributions; migration keeps existing single links                         | Migration + unit   | lint + typecheck + unit        | Apply in staging | Existing categories unchanged for users; entities can carry multiple categories; ancestors resolvable        |
| 18.2      | LLM enrichment     | Receipt intake attributes receipt + items into the taxonomy (extraction context grows candidate tree); manual override in review                   | Unit + integration | lint + typecheck + integration | Deploy           | New receipts arrive pre-attributed; review can correct; corrections feed back                                |
| 18.3      | Search backend     | Indexed search across transactions, receipts, items, comments, merchants (MySQL FULLTEXT or normalized search table); EN+HE analyzers              | Unit + integration | lint + typecheck + integration | Deploy           | Query returns matches from all entity kinds with entity type + link; Hebrew and English both match           |
| 18.4      | Search API         | `GET /search`: terms with include/exclude (`-term`), filters (date range, amount range, direction, categories incl. descendants), sort, pagination | API tests          | lint + typecheck + integration | Deploy           | Include/exclude honoured; category filter matches descendants; amount/date sort + filters correct            |
| 18.5      | Search UI          | Global search page + header omnibox; facet chips (category/date/amount/kind) with include/exclude toggles; result rows deep-link                   | UI tests           | lint + typecheck + unit        | Deploy           | Search reachable from anywhere; chips add/remove/negate filters; results link to transaction/receipt/comment |
| 18.6      | Relevance + locale | Ranking (recency + field weight), diacritics/plural folding, cross-language product matching reuse (Phase 8)                                       | Unit tests         | lint + typecheck + unit        | Deploy           | "bread" finds "לחם"-linked purchases via product links; best matches first                                   |
| 18.7      | Tests + polish     | E2E flows, a11y pass (combobox/listbox semantics), docs                                                                                            | E2E                | full suite                     | Deploy           | Keyboard-only search works; screen reader announces results; docs updated                                    |

### Phase 19: LLM Usage & Cost Tracking

> **Added (2026-07-15)**: by user request. Every LLM call the platform makes on a user's behalf is metered: provider (anthropic/openai today), model, token counts (input/output/cache) and computed cost, **linked to the entity that caused it** (receipt extraction, reconcile — and later agents, MCP tools, assistant chat). Matters most for BYOK users (Phase 8.11) who pay their own provider bills. Providers return token usage per response; monetary cost is computed from a per-model pricing map (kept configurable — providers don't expose a universal spend API).

| Iteration | Objective             | Scope                                                                                                                       | Testing            | CI/CD                          | Deployment       | Acceptance Criteria                                                                                |
| --------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------ | ---------------- | -------------------------------------------------------------------------------------------------- |
| 19.1      | Usage ledger schema   | `llm_usage_events`: user, provider, model, keySource (byok/shared), operation kind, token counts, cost, entityType+entityId | Migration + unit   | lint + typecheck + unit        | Apply in staging | Ledger rows queryable per user/provider/model/period                                               |
| 19.2      | Provider metering     | Both providers report usage from each response into the ledger (extraction already logs tokens — persist them)              | Unit tests         | lint + typecheck + unit        | Deploy           | Every extraction call writes exactly one ledger row with real token counts                         |
| 19.3      | Pricing map + cost    | Configurable per-model input/output/cache token prices; cost computed at write time; unknown model → tokens only            | Unit tests         | lint + typecheck + unit        | Deploy           | Costs match provider pricing for known models; unknown models degrade gracefully                   |
| 19.4      | Entity attribution    | Each operation carries its source entity (receipt id today); contract extends to future operations (agents, MCP, chat)      | Unit + integration | lint + typecheck + integration | Deploy           | From a receipt one can list its LLM operations and their cost; orphan operations impossible        |
| 19.5      | Usage API + dashboard | Per-user summaries (by provider/model/operation/period) + settings dashboard for BYOK users                                 | API + UI tests     | lint + typecheck + integration | Deploy           | BYOK user sees tokens + estimated spend per provider/model over time; totals reconcile with ledger |
| 19.6      | Tests + docs          | Integration coverage, retention policy decision, docs                                                                       | Integration        | full suite                     | Deploy           | Ledger writes never block/fail the metered operation; docs updated                                 |

### Phase 20: Accounts, Balances & Bank Sync

> **Added (2026-09-25)**: by owner request. The app tracks what is spent but not from where, and has no balance. This phase adds **accounts** (bank, card, cash, other — scoped like budgets, with a derived ledger balance and a bank-reported balance), **transfers** (a transaction between two own accounts that never counts as spending — the Israeli credit-card billing cycle in one row), and **bank sync**: statement lines imported from the bank or card issuer are reconciled against existing transactions (manual or receipt-born), created, recorded as transfers, or ignored — enrichment flows both ways and every prompt is skippable. Security ranks above automation: the first channel is a manual export parsed **in the browser**, the second a **user-run connector** pushing normalised lines with a scoped token; the server never receives a statement file or a bank credential. Open Banking is not available to an unlicensed personal app and server-side scraping is rejected outright. Prerequisite: a **UI kit** extracted from the existing components, so the new surfaces are built from primitives that replace today's hand-rolled shells. Feeds Phase 10.9 (low-balance alerts), 11, 12, 16.
>
> **Detailed design**: [`docs/phase-20-accounts-design.md`](docs/phase-20-accounts-design.md).

| Iteration | Objective                      | Scope                                                                                                                                                                                                                                                                                                               | Testing                     | CI/CD                          | Deployment | Acceptance Criteria                                                                                                                      |
| --------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ------------------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| 20.1      | UI kit                         | `Dialog`, `Select`, `Textarea`, `Checkbox`, `Card`, `Badge`, `PageHeader`, `EmptyState`, `Tabs`, `Stat` extracted from existing instances; every hand-rolled instance replaced; wiki updated                                                                                                                        | Web unit                    | lint + typecheck + unit        | Deploy     | No hand-rolled dialog shell, raw `<select>` or card shell remains under `apps/web/src`; existing specs and E2E stay green                |
| 20.2      | Accounts schema + API          | Shared types + institutions; additive migration (`accounts`, `account_imports`, `account_statement_lines`, `transactions.account_id` / `transfer_account_id`); `transfer` system category; `AccountModule` CRUD + ledger balance; transaction account/transfer fields, filter, exclusion in analytics and dashboard | Unit + integration          | lint + typecheck + integration | Deploy     | Accounts CRUD with the budgets role matrix; ledger balance equals opening + Σ countable rows; transfers never appear in spend totals     |
| 20.3      | Accounts UI                    | `/accounts` list + form dialog, `AccountSelect` in the transaction form and filters, dashboard overview, nav, i18n EN+HE                                                                                                                                                                                            | UI + E2E                    | lint + typecheck + unit        | Deploy     | A user creates a bank and a card account, assigns transactions, and sees correct balances on both the list and the dashboard             |
| 20.4      | Statement parsing + import API | Shared parser engine + presets (generic CSV, Hapoalim, Leumi, Isracard, Cal, Max), browser decoder, `POST /accounts/:id/imports` with fingerprint dedup, matcher, line decision endpoints, apply-suggestions                                                                                                        | Unit + integration          | lint + typecheck + integration | Deploy     | Re-importing an overlapping statement inserts zero duplicates; an exact-amount transaction within the date window is proposed as a match |
| 20.5      | Import & review UI             | Import wizard (file → preview → result), account detail with Review / Transactions / Imports tabs, fixture statements, Playwright flow                                                                                                                                                                              | UI + E2E                    | lint + typecheck + unit        | Deploy     | From a fixture XLSX a user imports, accepts suggestions, resolves a pending line and reaches a zero reconciliation gap                   |
| 20.6      | Two-way enrichment             | Auto-link on transaction create and receipt confirm, plan-occurrence settle on match, category memory, reconciliation gap on cards and dashboard                                                                                                                                                                    | Integration + E2E           | lint + typecheck + integration | Deploy     | A receipt confirmed after an import links to its pending line without a click; a matched pending occurrence becomes POSTED               |
| 20.7      | Connector                      | Scoped personal access tokens + settings UI; `apps/connector` CLI on `israeli-bank-scrapers` mapping to the import contract; security review                                                                                                                                                                        | Unit + integration + manual | lint + typecheck + integration | Deploy     | The connector pushes lines with a token that can do nothing else; revoking the token stops it; no credential exists server-side          |
| 20.8      | Balance alerts + release       | Per-account low-balance threshold feeding the 10.9 alert worker; progress docs; production release                                                                                                                                                                                                                  | Full suite                  | full suite                     | Deploy     | Production green; a balance below threshold raises exactly one alert per period                                                          |
...
- **Phase 6**: Unified Transaction Management (incomes + expenses in one entity with a `direction` field). Attribution to personal and/or groups; system + custom per-scope categories; one-time, recurring, limited-period, installment, loan, and mortgage types; per-user stars; comments; documents placeholder for Phase 7 receipts; amortisation; BullMQ recurring engine. Replaces original Phase 6 + Phase 7. (21 iterations — see [`docs/phase-6-transactions-design.md`](docs/phase-6-transactions-design.md).)
- **Phase 7**: Receipt ingestion & LLM extraction — upload (photo/PDF/URL), pluggable vision-LLM structured extraction (merchant, time, items, quantities, discounts), review flow, receipt → transaction with line items.
...
- **Phase 18**: Centralized search — hierarchical taxonomy, indexed search, omnibox.
- **Phase 19**: LLM usage & cost tracking — per-call ledger, pricing map, BYOK dashboard.
- **Phase 20**: Accounts, balances & bank sync — accounts and transfers, ledger vs reported balance, secure statement import and reconciliation, user-run connector; preceded by the UI kit.
...
| Transactions (unified) | Depends on groups and auth; replaces separate income + expense phases   | Backend API, BullMQ worker, and frontend components can parallelise |
| Receipts (7)           | Depends on transactions + file storage; extraction provider layer first | Upload UI can parallel with extraction worker                       |
...
| Budgets (10)           | Depends on transactions for progress tracking                           | Can start schema early                                              |
...
| Analytics ∥ Budgets     | Phase 9 (Analytics) ∥ Phase 10 (Budgets)      | Phase 8 / 6 complete   | Budgets only needs transactions; can start any time after 6 |
...
gantt
    ...
    Phase 6 - Transaction Management   :p6, after p5, 28d
...
1. Phase 0 → Phase 1 → Phase 4 → Phase 5 → Phase 6 (Transactions) → Phase 7 (Receipts) → Phase 8 (Products) → Phase 9 (Analytics) → Phase 10 (Budgets)
