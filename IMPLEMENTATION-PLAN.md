...
| Phase 7: Receipt Ingestion & LLM Extraction               | 13         | Medium        | Pluggable vision-LLM extraction; receipts/items/merchants schema; review flow; URL ingestion; follow-ups: HEIC→JPEG, readable URL snapshots, payment-first intake (receipt = payment document)                     |
...
>
> 1. **HEIC (7.11)**: the upload whitelist accepts `image/heic` (the iPhone camera default), but vision LLM APIs reject HEIC and browsers cannot render it in previews — so the platform's most likely mobile upload failed extraction _and_ preview. Fix: convert HEIC → JPEG once, at storage time (WASM decoder — no native image libs in the container); everything downstream (extraction, preview, `PaymentDocument`) sees a JPEG.
...
> 3. **Payment-first intake (7.13)**: receipts and payments lived as parallel top-level objects, but a receipt is not a peer of a payment — it is an **attribute of a payment**: the document that proves it and carries its details. Primary intake moves into the payment interface (**Add payment → From receipt**), which uploads the file and hands off to the existing extract → review → confirm pipeline that ends in the payment; the payment detail page links back to its source receipt. `/receipts` remains as the pipeline/history view of in-flight documents.

| Iteration | Objective                    | Scope                                                                                                                                                                                                           | Testing        | CI/CD                   | Deployment | Acceptance Criteria                                                                                                          |
| --------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ----------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 7.11      | HEIC → JPEG at storage       | Convert `image/heic` uploads to JPEG in the storage service (WASM decode, quality ~0.9); stored artifact, preview, extraction input, and `PaymentDocument` all become JPEG; corrupt HEIC → structured 400       | Unit tests     | lint + typecheck + unit | Deploy     | An iPhone HEIC upload extracts and previews exactly like a JPEG upload                                                       |
...
| 7.13      | Payment-first receipt intake | "From receipt" action in the Add-payment dialog: upload → redirect to review → confirm lands back on the payment; payment detail shows a link to its source receipt (`receiptId` on the detail DTO); i18n EN+HE | UI + API tests | lint + typecheck + unit | Deploy     | User starts from Add payment, uploads, reviews, confirms; the resulting payment links back to its receipt in both directions |
...
> **Follow-ups (2026-07-12/13)** — receipt intake & payment integration; detailed design: `docs/phase-8-receipt-intake-design.md`.
...
| 8.13      | Intake chooser            | "From receipt" in Add Payment becomes a chooser: upload from device / **add from URL**                                                                                                        | UI tests           | lint + typecheck + unit        | Deploy     | A URL pasted in the Add Payment dialog lands in the extract → review → confirm pipeline         | shipped |
...
| 8.15      | Attach + reconcile        | Attach receipt (file/URL) to an existing payment; after extraction a reconciliation dialog offers keep/update per field (total, category); items/products update regardless                   | UI + integration   | lint + typecheck + integration | Deploy     | Attaching a receipt to a payment updates products always and the payment header only on request | shipped |
| 8.16      | Receipt⇄payment invariant | No receipt without a payment: receipts directory mirrors payments; pre-confirm receipts presented as transient intake; stale-draft cleanup                                                    | Full test suite    | full suite                     | Deploy     | Every listed receipt links its payment; orphan receipts no longer exist as a durable state      | planned |
