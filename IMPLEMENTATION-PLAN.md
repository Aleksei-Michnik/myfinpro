...
| 8.14      | Manual receipt by barcode | Camera scanning composes a receipt: per scan → product lookup/create, quantity + unit price per line, price memory (same barcode → qty bump; history prefills price); `POST /receipts/manual` | UI + integration   | lint + typecheck + integration | Deploy     | A receipt can be composed by scanning products and lands in review with items pre-linked        | planned |
