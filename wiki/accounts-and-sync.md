...
- Push: chunks of `ACCOUNT_IMPORT_MAX_LINES`, `source` = the company mapped onto `ACCOUNT_IMPORT_SOURCES` (`visaCal` → `cal`, the seven others by name, everything else `connector`), no `originalName`. Exit codes 0/1/2/3/4 = ok / usage / config / scrape / API, with 401 ⇒ "run init".
