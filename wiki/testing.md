...
| `backup.yml`            | cron daily 02:17 UTC + dispatch (`drill`)                                  | per environment on the server: `backup.sh` → `verify-backup.sh` (restore drill on Sundays) → `check-backup-age.sh --max-age 26`; a red run is the backup alert                                                                     |
| `backup-verify.yml`     | cron Sun 03:00 UTC + dispatch                                              | CI drill of the same scripts against a throwaway `mysql:9.7` service: backup → verify with restore drill → restore → row counts → both outcomes of the age check                                                                   |
