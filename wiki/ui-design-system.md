...
cannot be overridden. Typography beyond the font stack is not tokenised: sizes/weights are picked
per component from Tailwind's scale (`Button` `sizeStyles`: `sm` `text-sm`, `md` `text-base`,
`lg` `text-lg`).
...
| `Input`              | Labelled text input with error state                                                                      | `label`, `error`; forwards ref + all input attrs                                                 |
| `input-styles.ts`    | `inputClass` — compact control style for dense editing surfaces (receipt review, walkthrough)             | — (a string, not a component)                                                                    |
...
| `ConfirmDialog`      | THE generic destructive-action confirm modal                                                              | `title`, `message`, `confirmLabel`, `cancelLabel`, `danger`, `busy`                              |
...
| Form dialog      | `components/transaction/TransactionFormDialog.tsx` (control-scope save, per-field domain errors, inline banner for transport errors)                                                                                                                        |
