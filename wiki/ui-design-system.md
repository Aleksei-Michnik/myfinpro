...
**Drift:** no `data-theme` attribute, no pre-hydration theme script, no in-app theme switch
(nothing under `src/app/[locale]/settings` mentions theme) — dark mode follows the OS and cannot be
overridden. Typography beyond the font stack is not tokenised: sizes/weights come per component
from Tailwind's scale (`Button` `sizeStyles`: `sm` `text-sm`, `md` `text-base`, `lg` `text-lg`).
...
| `ReceiptIntake` (`components/receipt/`)   | THE receipt intake on every host (`/receipts`, transaction form, attach dialog): camera + browse + drop + URL + host extras, gate, staging, create — `docs/ui/8.29-receipt-intake.md` | `target` `standalone\|transaction`, `onCreated(receipts)`, `onScanBarcodes?`, `onLinkExisting?`, `hint`, `disabled`, `testIdPrefix` |
| `StagedPagesTray` (`components/receipt/`) | The staged photos of one receipt: ordered thumbnails, per-page remove, upload as one / separately / clear                                                                             | `files`, `pending`, `canUploadSeparately`, `testIdPrefix`, `onRemove`, `onUploadOne`, `onUploadSeparately`, `onClear`               |
...
- Tailwind resolves competing utilities by stylesheet order, not string order: never hand a kit
  ...
  (`padding`, `size`, `fullWidth`), `wrapperClassName="contents"` when a control must add no box.
