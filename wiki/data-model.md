...
| ------------------------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Transaction`            | one money movement, IN or OUT           | `direction` `IN`\|`OUT`, `type`, `amountCents`, `currency`, `occurredAt`, `status` (default `POSTED`), `categoryId` (the **primary** category), self-relation `parent`/`occurrences`, `idempotencyKey?` unique, `accountId?` / `transferAccountId?` (both SetNull, Phase 20) | `(direction, occurredAt)`, `categoryId`, `(createdById, occurredAt)`, `parentTransactionId`, `(type, status)`, `(accountId, occurredAt)`, `(transferAccountId, occurredAt)` |
...
## Accounts and bank sync

| Model                  | Purpose                               | Key fields                                                                                                                                                                                                                                                                                                      | Indexes / uniques                                                                                        |
| ---------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `Account`              | bank account, card, cash wallet       | `kind` `BANK`\|`CARD`\|`CASH`\|`OTHER`, `institution?`, `currency`, `last4?` (masked only), `scopeType` + exactly one of `ownerId`/`groupId`, anchor `openingBalanceCents`/`openingBalanceAt`, bank-stated `reportedBalance*`, CARD-only `billingAccountId?` (self-relation) + `billingDay?` 1–28, `archivedAt` | `(ownerId,scopeType,archivedAt)`, `(groupId,scopeType,archivedAt)`                                       |
| `AccountImport`        | one statement import (20.4)           | `source` (one of `ACCOUNT_IMPORT_SOURCES`), `originalName?`, `periodFrom/To?`, `statementBalance*?`, frozen counters `totalCount`/`insertedCount`/`duplicateCount`. The file itself is **never** stored                                                                                                         | `(accountId, createdAt)`                                                                                 |
| `AccountStatementLine` | one bank line — the bank truth (20.4) | `fingerprint` (sha256 dedup fence), `postedAt`/`valueAt?`, `direction`, `amountCents`, `description` + `normalizedDescription`, card extras (`originalAmount*`, `installment*`), `status` `PENDING`\|`MATCHED`\|`CREATED`\|`IGNORED`, `transactionId?` unique, `suggestion Json?`                               | `@@unique([accountId, fingerprint])`, `(accountId,status,postedAt)`, `(accountId,normalizedDescription)` |

The balance is **derived**, never stored: `openingBalanceCents` + Σ countable rows
(`status='POSTED' AND type='ONE_TIME'`, `occurredAt >= openingBalanceAt`) signed by direction,
plus incoming transfers (`transferAccountId`). Deleting an account SetNulls its transactions and
cascades its imports and lines. Iteration 20.2 ships `accounts` only — the two sync tables exist so
the migration ships once. Domain page: [accounts-and-sync.md](accounts-and-sync.md).
...
- `Budget` and `Account` mirror attributions with `scopeType` + exactly one of `ownerId`/`groupId`;
  an `AccountImport` / `AccountStatementLine` is reached only through its account.
