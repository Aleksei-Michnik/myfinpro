...
> **Depends on**: Phase 6 (Payment entity, attributions, categories, PaymentDocument, BullMQ, realtime SSE stack)
...
creates a regular Phase-6 `Payment` (direction `OUT`) carrying the receipt
file as a `PaymentDocument` and the line items as first-class rows for
...
| Multi-receipt per payment, receipt for IN payments | Out of scope — one receipt = one OUT payment          |
...
- `CONFIRMED` — terminal; `paymentId` set. Items frozen (Phase 8 matching
...
rows). Confirmed receipts are deleted only via their payment (Phase 6
delete semantics), which nulls `receipts.payment_id` (SetNull) and returns
the receipt to `REVIEW`… **no** — kept simple: deleting the payment
cascades a `receipts.payment_id → SetNull`, the receipt stays `CONFIRMED`
as an audit artifact and can be re-confirmed. (Cheap, avoids a special
state.)

### 2.2 One receipt → one payment

Confirming creates ONE `Payment` (`direction=OUT`, `type=ONE_TIME`,
`occurredAt = purchasedAt`, `amountCents = totalCents`) with the user's
chosen attributions (remembered scopes, same UX as the payment form), plus
a `PaymentDocument` (`kind='receipt'`, `fileRef` pointing at the SAME
stored file — no duplication). The payment's category is the user-picked
...
confirmed; the confirm flow's payment fan-out reuses the Phase 6
`payment.created` event). Views follow `docs/ui-realtime-conventions.md`
...
model Receipt {
  ...
  paymentId     String?   @unique @map("payment_id") @db.VarChar(36) // SetNull on payment delete
  createdAt / updatedAt
...
| POST   | `/receipts/:id/confirm` | REVIEW → CONFIRMED: creates merchant?, Payment, PaymentDocument; links paymentId |
...
Confirm validates like `POST /payments` (currency list, category
...
- **`/receipts/[receiptId]`** — review page: file preview (image/PDF
  ...
  Delete.
- Confirmed receipts deep-link to the payment detail page; the payment
  detail page's documents section lists the receipt file (replaces the
  Phase 6 placeholder for these payments).
...
| 7.9  | Confirm flow → payment + document; payment detail receipt link                                                          |
