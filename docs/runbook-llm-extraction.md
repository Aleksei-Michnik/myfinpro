...
| **iPhone photos** fail / go `FAILED`                                                 | **HEIC not supported by the vision API** (the upload whitelist accepts HEIC, but Claude/OpenAI vision reject `image/heic`) | Upload **JPEG/PNG/PDF** instead. On iOS, Settings → Camera → Formats → **Most Compatible** to shoot JPEG. (HEIC→JPEG conversion is a tracked follow-up.) |
...
| **URL receipts** recognize poorly                                                    | The fetcher sends **raw HTML** (first 200k chars) to the model — noisy vs. an image                                        | Prefer uploading a photo/PDF; URL ingestion is best-effort.                                                                                              |
